...
- Antigravity now lists `gemini-3.8-flash-{high,medium,low,tiered}` even when Cloud Code Assist `fetchAvailableModels` omits them. Live `streamGenerateContent` still 404s those user-facing ids (`Requested entity was not found`), so the Antigravity transport remaps 3.7/3.8 Flash suffix ids onto the real `gemini-3.7-flash-tiered` CCA model and applies `thinkingLevel` from the suffix. `gemini-3.6-flash-*` remain unaliased. Sandbox catalog `baseUrl` values prefer daily Cloud Code Assist with a sandbox fallback.
- Auth-broker snapshot entries now accept optional/nullable `revision` on the strict wire schema, and local snapshot export includes it. Live brokers already emit this monotonic row revision (or omit/null it right after refresh); clients that rejected unknown keys failed `gjc accounts list` / gateway status with "Auth broker response failed schema validation".
