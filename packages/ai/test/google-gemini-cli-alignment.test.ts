...
describe("Google Gemini CLI alignment", () => {
	...
	describe("retry guardrails", () => {
		...
		it("does not treat explicit HTTP failures as network retry errors", async () => {
			...
			expect(result.transportFailure).toMatchObject({ kind: "transport", status: 503, providerCode: "server_error" });
			expect(result.transportFailure?.headers instanceof Headers ? result.transportFailure.headers.get("retry-after") : undefined).toBe("120");
