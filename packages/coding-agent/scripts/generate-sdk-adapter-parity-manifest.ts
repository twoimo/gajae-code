...
const commands = [...new Set([...Object.values(adapterTests).flat(), "packages/coding-agent/test/sdk-host-wiring.test.ts"])].sort();
...
function adapterTestPrefix(adapter: Adapter): string {
	return adapter === "telegram" ? "T" : adapter === "discord" ? "D" : adapter === "slack" ? "S" : adapter === "daemonCli" ? "L" : adapter === "mcp" ? "M" : "A";
}
function row(adapter: Adapter, operation: (typeof OPERATIONS)[number], secret = false): ManifestAdapterRow {
	...
	return {
		...
		argv: ["bun", "test", dispositionTestFile, "--test-name-pattern", `^AD-${adapterTestPrefix(adapter)}-${operation.id}${suffix}:`],
...
function generateRows(): ManifestAdapterRow[] {
	return OPERATIONS.flatMap(operation =>
		adapters.flatMap(adapter => [row(adapter, operation), ...(operation.id === "C36" ? [row(adapter, operation, true)] : [])]),
	);
}

function generateManifest(): Manifest {
	return { version: 1, commands: commands.map(file => ({ argv: ["bun", "test", file] })), excluded, required, rows: generateRows() };
}
...
async function checkCoverage(manifest: Manifest): Promise<void> {
	...
	for (const row of expectedRows) {
		if (!rowKeys.has(row.adapterTestId)) throw new Error(`${row.adapterTestId} has no adapter-disposition manifest row.`);
	}
	...
	for (const row of rows) {
		...
		if (/placeholder|todo|tbd|example/i.test(row.testNamePattern)) throw new Error(`${row.adapterTestId} uses a placeholder test name pattern.`);
		if (row.argv.length !== 5 || row.argv[0] !== "bun" || row.argv[1] !== "test" || row.argv[2] !== row.testFile || row.argv[3] !== "--test-name-pattern")
		...
		if (!OPERATIONS.some(operation => row.sdkId === operation.sdkId)) throw new Error(`${row.adapterTestId} references an unknown SDK ID.`);
	}
...
async function checkManifest(expected: Manifest): Promise<void> {
	...
	if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new Error(`SDK adapter parity manifest is stale: ${path.relative(repoRoot, manifestPath)}`);
	await checkCoverage(actual);
}

const expected = validateManifest(generateManifest());
if (process.argv.slice(2).includes("--check")) {
	try { await checkManifest(expected); } catch (error) { process.stderr.write(`${error instanceof Error ? error.message : String(error)}\n`); process.exit(1); }
} else {
	await Bun.write(manifestPath, `${JSON.stringify(expected, null, "\t")}\n`);
	await checkCoverage(expected);
	process.stderr.write(`Generated ${path.relative(repoRoot, manifestPath)} (${expected.rows?.length ?? 0} row receipts)\n`);
}
