...
async function sessionMoveModel(thread: ThreadRecord, params: unknown) {
	...
	try {
		...
		if (path.resolve(targetSessionFile) !== path.resolve(sourceSessionFile))
			conflicts.push("target session file already exists");
	} catch {}
	const crossDevice = path.parse(sourceSessionFile).root !== path.parse(targetCwd).root;
	if (record.dryRun === true)
		return { dryRun: true, sourceSessionFile, targetSessionFile, artifactsDirs, crossDevice, conflicts };
...
async function modelAssignModel(session: AgentSessionLike, params: unknown) {
	...
	if (role === "default") {
		if (typeof session.setModel === "function")
			await session.setModel(model, role, { selector: baseSelector, ...(thinkingLevel ? { thinkingLevel } : {}) });
		...
		const materializedProfile = materializeActiveModelProfileAssignment({
			session: session as never,
			settings: settings as never,
			role: role as GjcModelAssignmentTargetId,
			selector: persistedSelector,
		});
...
async function requireExtensionId(cwd: string, extensionId: string): Promise<void> {
	...
	if (!extensions.some(extension => extension.id === extensionId))
...
function jobChangeDescription(job: Record<string, unknown>): string | undefined {
	...
	return (
		optionalString(asRecord(metadata.subagent).description) ??
		optionalString(metadata.description) ??
		optionalString(job.label)
	);
}
...
const SECRET_QUERY_PARAM_PATTERN =
	/([?&](?:state|code|code_verifier|client_secret|access_token|refresh_token|id_token|token|fingerprint)=)[^&#\s]+/gi;
...
function redactObjectEntries(record: Record<string, unknown>): Record<string, unknown> {
	...
	const usedKeys = new Set<string>();
	for (const [key, item] of Object.entries(record))
		next[sanitizedKeyFor(key, usedKeys)] = redactExportValue(item, key);
	return next;
}
...
export class AgentSessionHost implements AppServerHost {
	...
	async providerAdd(params: unknown): Promise<unknown> {
		return providerAddModel(
			params,
			[...this.#threads.values()].map(thread => thread.session),
		);
	}
	...
	async authLoginStart(params: unknown): Promise<unknown> {
		const providerId = optionalString(asRecord(params).providerId);
		if (!providerId) throw new Error("providerId is required");
		const storage = (await this.#connectionAuthStorage()) as AuthStorage & {
			login?: (providerId: string, callbacks: Record<string, unknown>) => Promise<unknown>;
		};
		...
		const flow: {
			...
			inputSlot?: (value: string) => void;
			abort?: AbortController;
		} = { providerId, state: "idle", abort };
		this.#loginFlows.set(flowId, flow);
		const audit = (state: string) =>
			console.info(JSON.stringify({ event: "gjc.auth.login", providerId, flowId, state }));
		...
		void storage
			...
			.then(
				() => {
					flow.state = abort.signal.aborted ? "cancelled" : "authenticated";
					audit(flow.state);
					markFirstTransition();
				},
				() => {
					flow.state = abort.signal.aborted
						? "cancelled"
						: flow.state === "unsupported"
							? "unsupported"
							: "failed";
		...
		return {
			...
			...(flow.promptMessage ? { instructions: flow.promptMessage } : {}),
		};
	}
	...
	async authLoginComplete(params: unknown): Promise<unknown> {
		...
		console.info(
			JSON.stringify({
				event: "gjc.auth.login",
				providerId: flow.providerId,
				flowId: optionalString(record.flowId),
				state: flow.state,
			}),
		);
	...
	async pluginsSetFeature(params: unknown): Promise<unknown> {
		...
		if (!pluginId || !feature || typeof record.enabled !== "boolean")
			throw new Error("pluginId, feature, and enabled are required");
		const manager = new PluginManager();
