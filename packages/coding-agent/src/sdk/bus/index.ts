...
import { dispatchControl, type ControlSurface } from "../host/control";
...
class DiffQueryError extends Error {
	constructor(readonly code: "not_git_repository" | "diff_too_large", message: string) {
		super(message);
	}
}
	...
	for (const definition of definitions as unknown[]) {
		...
		if (capability === "host_tools") {
			...
			if (typeof entry.description !== "string")
				invalid("host_tools definitions require a string description.");
...
const UNINSTALLED_CONTROL_OPERATIONS = new Set([
	"auth.login",
	"host_tools.register",
	"host_uri.register",
]);
...
const CONTROL_BINDINGS: Readonly<Record<string, string | undefined>> = {
	...
	"skill.invoke": "invokeSkill", "mode.plan.set": "setPlanMode", "mode.goal.operate": "operateGoal",

	"compaction.auto.set": "sdkControl", "retry.auto.set": "sdkControl", "retry.abort": "sdkControl",
	"bash.execute": "sdkControl", "bash.abort": "sdkControl",
	"session.new": "sdkControl", "session.fork": "sdkControl", "session.resume": "sdkControl", "session.close": "sdkControl", "session.switch": "sdkControl", "session.branch": "sdkControl", "session.rename": "sdkControl", "session.handoff": "sdkControl", "session.export_html": "sdkControl",
	"runtime.reload": "sdkControl", "service_tier.set": "sdkControl", "queue.message.remove": "sdkControl", "queue.message.move": "sdkControl", "queue.message.update": "sdkControl", "extension.set_enabled": "sdkControl", "session.delete": "sdkControl", "session.cwd.move": "sdkControl",
	"retry.last": "sdkControl", "retry.now": "sdkControl", "bash.background": "sdkControl",
};
...
function installedOperations(ctx: ExtensionContext, kind: "control" | "query"): Set<string> {
	...
	const candidates = OPERATIONS.filter(operation =>
		...
		(kind !== "control" || (
			...
			((operation.sdkId !== "workflow.gate_answer" && operation.sdkId !== "workflow.plan_approve") || ctx.workflowGate !== undefined) &&
...
function sdkQuerySurface(
	...
	const getDiff = async () => {
		try {
			const { stdout } = await execFileAsync("git", ["diff", "--no-ext-diff"], { cwd: ctx.cwd, maxBuffer: 1024 * 1024 });
			return stdout.split(/^diff --git /m).filter(Boolean).map(section => {
	...
	return {
		getTranscriptEntries: () => (typeof (ctx as Partial<ExtensionContext>).getTranscript === "function" ? ctx.getTranscript() : []),
		getContextSnapshot: () => ({ usage: ctx.getContextUsage(), systemPrompt: ctx.getSystemPrompt(), ...getLiveState() }),
		getGoalState: () => (typeof (ctx as Partial<ExtensionContext>).getGoalState === "function" ? ctx.getGoalState() : undefined),
		getTodoState: () => (typeof (ctx as Partial<ExtensionContext>).getTodoState === "function" ? ctx.getTodoState() : []),
		getDiff,
		getUsage: () => ctx.sessionManager.getUsageStatistics(),
		getModels: () => ctx.modelRegistry.getAll().map(model => ({ provider: model.provider, id: model.id, name: model.name, contextWindow: model.contextWindow, maxTokens: model.maxTokens })),
...
function containsSecretConfigKey(value: unknown, seen = new Set<object>()): boolean {
	...
	return Object.entries(value as Record<string, unknown>).some(
		([key, nested]) => /(?:token|secret|password|api[_-]?key|credential|authorization)/i.test(key) || containsSecretConfigKey(nested, seen),
	);
}
...
function sdkControlSurface(
	...
	const resolveModel = (id: string) => {
		...
		const model = modelId.length > 0 ? ctx.modelRegistry.find(provider, modelId.join("/")) : ctx.modelRegistry.getAll().find(candidate => candidate.id === id);
		if (!model) throw Object.assign(new Error(`Model ${id} was not found.`), { code: "invalid_input" });
		return model;
	};
	const unavailablePerSession = (operation: string) => unavailable(operation, "the registry classifies it outside the per-session extension host");
	const typed = (operation: string, input: Record<string, unknown> = {}) => {
		if (!bindings.has("sdkControl") || !ctx.sdkControl) return unavailable(operation, "no typed session seam is installed")();
		return ctx.sdkControl(operation, input);
	};
	const surface: ControlSurface = {
		prompt: (text, images) => { const content: string | (TextContent | ImageContent)[] = Array.isArray(images) && images.length > 0 ? [{ type: "text", text } as TextContent] : text; api.sendUserMessage(content, isBusy() ? { deliverAs: "steer" } : undefined); return { commandId: crypto.randomUUID(), accepted: true }; },
		steer: text => send(text, "steer"), followUp: text => send(text, "followUp"),
		abort: () => { ctx.abort(); return { aborted: true }; }, abortAndPrompt: text => { ctx.abort(); return send(text); },
		answerAsk: (id, answer) => { const pending = pendingInteractive.get(id); if (!pending) throw Object.assign(new Error(`Ask ${id} was not found.`), { code: "resource_gone" }); pendingInteractive.delete(id); pending.resolve(mapAnswerToLabel(JSON.stringify(answer), pending.options)); return { resolved: true }; },
		answerGate: (id, response) => ctx.workflowGate?.resolveGate?.({ gate_id: id, answer: response, idempotency_key: id }) ?? unavailable("workflow.gate_answer", "workflow gates are unavailable for this session")(),
		approvePlan: (id, choice) => ctx.workflowGate?.resolveGate?.({ gate_id: id, answer: choice, idempotency_key: id }) ?? unavailable("workflow.plan_approve", "workflow gates are unavailable for this session")(),
		invokeSkill: (name, args) => {
			if (!bindings.has("invokeSkill") || !ctx.invokeSkill) return unavailable("skill.invoke", "no skill invocation seam is installed")();

			if (typeof args !== "undefined" && typeof args !== "string") throw Object.assign(new Error("skill.invoke args must be a string."), { code: "invalid_input" });
			return ctx.invokeSkill(name, args);
		},
		setPlanMode: on => {
			if (!bindings.has("setPlanMode") || !ctx.setPlanMode) return unavailable("mode.plan.set", "no plan-mode seam is installed")();
		...
		operateGoal: (op, objective) => {
			if (!bindings.has("operateGoal") || !ctx.operateGoal) return unavailable("mode.goal.operate", "no goal-mode seam is installed")();
			if (!["create", "get", "resume", "pause", "complete", "drop"].includes(op)) throw Object.assign(new Error("mode.goal.operate requires a supported op."), { code: "invalid_input" });
			if (objective !== undefined && typeof objective !== "string") throw Object.assign(new Error("mode.goal.operate objective must be a string."), { code: "invalid_input" });
		...
		cycleModel: async () => { if (!bindings.has("cycleModel")) return unavailable("model.cycle", "no session model-cycle seam is installed")(); return { changed: (await ctx.cycleModel()) !== undefined }; },
		setThinking: level => { api.setThinkingLevel(level as ThinkingLevel); return { changed: true }; },
		cycleThinking: () => { if (!bindings.has("cycleThinkingLevel")) return unavailable("thinking.cycle", "no session thinking-cycle seam is installed")(); return { level: ctx.cycleThinkingLevel() }; },
		setPermissionMode: mode => typed("permission_mode.set", { mode }),
		setQueueMode: (kind, mode) => { if (!bindings.has("setQueueMode")) return unavailable(`queue.${kind}_mode.set`, "no session queue-mode seam is installed")(); if (!ctx.setQueueMode(kind as "steering" | "follow_up" | "interrupt", mode)) throw Object.assign(new Error("Invalid queue mode."), { code: "invalid_input" }); return { changed: true }; },
		runCompaction: async () => { try { await ctx.compact(); return { started: true }; } catch (error) { throw Object.assign(new Error(error instanceof Error ? error.message : "Compaction is unavailable for the current state."), { code: "invalid_request" }); } },
		setAutoCompaction: on => typed("compaction.auto.set", { on }), setAutoRetry: on => typed("retry.auto.set", { on }), abortRetry: () => typed("retry.abort"),
		executeBash: cmd => typed("bash.execute", { cmd }), abortBash: () => typed("bash.abort"), newSession: () => typed("session.new"), forkSession: () => typed("session.fork"), resumeSession: id => typed("session.resume", { id }), closeSession: () => typed("session.close"), switchSession: id => typed("session.switch", { id }), branchSession: entryId => typed("session.branch", { entryId }), renameSession: name => typed("session.rename", { name }), handoffSession: target => typed("session.handoff", { target }), exportHtml: () => typed("session.export_html"),
		patchConfig: patch => { if (!patch || typeof patch !== "object" || Array.isArray(patch)) throw Object.assign(new Error("config.patch requires an object."), { code: "invalid_input" }); if (containsSecretConfigKey(patch)) throw Object.assign(new Error("config.patch rejects secret fields at the SDK host."), { code: "invalid_input" }); if (!settings) return unavailable("config.patch", "configuration settings are unavailable for this session")(); const entries = Object.entries(patch as Record<string, unknown>); for (const [key, value] of entries) settings.set(key as never, value as never); for (const [key, value] of entries) configOverrides.set(key, value); configRevision.current += 1; return { patched: entries.map(([key]) => key), revision: String(configRevision.current) }; },


		reloadRuntime: components => typed("runtime.reload", { components }), login: unavailablePerSession("auth.login"), registerHostTools: unavailablePerSession("host_tools.register"), registerHostUri: unavailablePerSession("host_uri.register"), setServiceTier: tier => typed("service_tier.set", { tier }),
		setActiveTools: async names => { await api.setActiveTools(Array.isArray(names) ? names.filter((name): name is string => typeof name === "string") : []); return { changed: true }; },
		removeQueueMessage: id => typed("queue.message.remove", { id }), moveQueueMessage: (id, position) => typed("queue.message.move", { id, ...position }), updateQueueMessage: (id, patch) => typed("queue.message.update", { id, patch }), setExtensionEnabled: (id, on) => typed("extension.set_enabled", { id, on }),
		clearContext: async confirm => { if (!confirm) throw Object.assign(new Error("context.clear requires confirmation."), { code: "confirmation_required" }); return { cleared: await ctx.clearContext() }; },
		deleteSession: (id, confirm) => { if (!confirm) throw Object.assign(new Error("session.delete requires confirmation."), { code: "confirmation_required" }); return typed("session.delete", { id }); }, moveCwd: path => typed("session.cwd.move", { path }), retryLast: () => typed("retry.last"), retryNow: () => typed("retry.now"), backgroundBash: () => typed("bash.background"),
...
export function createNotificationsExtension(
	...
	async function startSession(ctx: ExtensionContext): Promise<"started" | "already" | "disabled" | "failed"> {
		...
		const installProviderDefinitions = (capability: string, definitions: unknown) => {
			...
			ctx.setSdkPermissionProvider?.(async (toolCall, permissionOptions, signal) => {
				...
				if (!result || typeof result !== "object") throw new Error("permission provider returned an invalid response");
				...
				if (response.outcome === "selected" && typeof response.optionId === "string")
					return { outcome: "selected", optionId: response.optionId, ...(typeof response.kind === "string" ? { kind: response.kind as "allow_once" | "allow_always" | "reject_once" | "reject_always" } : {}) };
		...
		const controlSurface = sdkControlSurface(ctx, pendingInteractive, api, () => runtime?.busy === true, settings, configOverrides, configRevision);
		...
		server.onReply((err, reply) => {
			...
			// 2) Workflow gate: resolve the real gate, then confirm.
			if (workflowGates && gate?.resolveGate) {
		...
		try {
			...
			runtime.enableNotifications = () => {
				...
				if (workflowGates && gate?.onGateEmitted)
					runtime.disposeGateListener = gate.onGateEmitted(g => {
						...
						try {
							server.registerAsk(
								JSON.stringify(
									notificationActionPayload(
										{ id: g.gate_id, kind: "ask", sessionId: runtime.id, question, options },
										{ redact: runtime.redact, sessionTag: tag },
									),
								),
								true,
							);
			...
			logger.info(`notifications: serving session ${id} at ${endpoint.url} (workflowGates=${workflowGates})`);

			if (notificationsEnabledForSession && settingsAvailable && settings) {
				try {
					if (isTelegramConfigured(cfg)) await ensureTelegramDaemonRunning({ settings, cwd: ctx.cwd, sessionId: id });
	...
	api.on("turn_start", (_event, ctx) => {
		const id = sessionId(ctx);
		const rt = runtimes.get(id);
		if (!rt || !rt.notificationsActive) return;
	...
	api.on("tool_execution_start", (event, ctx) => {
		...
		const id = sessionId(ctx);
		const rt = runtimes.get(id);
		if (!rt || !rt.notificationsActive || rt.redact) return;
