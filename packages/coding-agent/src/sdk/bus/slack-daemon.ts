...
import { acceptsSlackInbound, normalizeSlackConversation, slackConversationKey, type SlackConversation, type SlackInboundDispatchReceipt } from "./slack-conversation";
...
import { SlackProvider, type SlackPostedMessage, type SlackSocketEnvelope } from "./slack-provider";
import { SlackProviderError } from "./slack-live-provider";
import { ChatEffectJournal, type ChatEffect, type ChatEffectLease } from "./chat-effect-journal";
...
export interface SlackNotificationDaemonOptions {
	...
	onCommand?: (sessionId: string, content: string, endpoint: SlackEndpoint, idempotencyKey: string) => Promise<boolean>;
...
type EventsPayload = { type?: unknown; event_id?: unknown; team_id?: unknown; event_context?: unknown; event?: SlackEvent };
...
function messageFromEnvelope(payload: unknown): { eventId: string; eventContext?: string; teamId: string; channelId: string; rootTs: string; event: SlackEvent } | undefined {
	if (!payload || typeof payload !== "object") return undefined;
	const eventPayload = payload as EventsPayload;
	if (eventPayload.type !== "events_api" || !eventPayload.event || eventPayload.event.type !== "message") return undefined;
	...
	return { eventId, eventContext: text(eventPayload.event_context), teamId, channelId, rootTs, event: eventPayload.event };
}
...
export class SlackNotificationDaemon {
	...
	constructor(private readonly options: SlackNotificationDaemonOptions) {
		this.store = options.store ?? new ConversationStore<SlackConversation>({ agentDir: options.agentDir, kind: "slack" });
		...
		this.#resolveEndpoint = options.resolveEndpoint ?? (async sessionId => {
	...
	async start(): Promise<void> {
		...
		try {
			...
			await this.options.provider.start(async envelope => { await this.#track(this.handleEnvelope(envelope)); });
		} catch (error) {
			this.#started = false;
	...
	async handleEnvelope(envelope: SlackSocketEnvelope): Promise<boolean> {
		...
		if (inbound.event.bot_id || inbound.event.subtype === "bot_message" || inbound.event.user === this.options.botUserId) {
			await this.options.provider.ack(envelope.envelope_id);
			return false;
		}
	...
	async postRoot(sessionId: string, body: string, endpointGeneration?: number): Promise<SlackConversation> {
		...
		const pending = await this.store.transact(pendingKey, current => {
			...
			if (current?.state === "posting_root") {
				if (current.rootPublicationOwner === this.#publicationOwnerId || !this.#leaseExpired(current.rootPublicationLeaseExpiresAt, now)) return current;
				claimed = true;
		...
		try {
			await this.#withRootLease(pendingKey, clientMsgId, fence, async () => {
				posted = await this.#postDurable(`root:${sessionId}:${clientMsgId}`, sessionId, endpointGeneration ?? 0, { channel: this.options.channelId, text: body, clientMsgId });
			});
		} catch (error) {
			...
			if (!posted) {
				if (!this.#isUncertainPostFailure(error)) {
					await this.store.transact(pendingKey, current => current?.clientMsgId === clientMsgId && current.rootPublicationOwner === this.#publicationOwnerId && current.rootPublicationFence === fence
						? nextRecord(current, { state: "error", rootPublicationOwner: undefined, rootPublicationLeaseExpiresAt: undefined, updatedAt: this.#now(), lastError: "provider_failure" })
						: current);
				}
				throw error;
			}
		...
		const endpoint = await this.#withRootLease(pendingKey, clientMsgId, fence, async () => await this.#resolveEndpoint(sessionId));
		const generation = endpointGeneration ?? endpoint?.generation;
		const active = await this.store.transact(pendingKey, current => {
			if (!current || current.clientMsgId !== clientMsgId || current.rootPublicationOwner !== this.#publicationOwnerId || current.rootPublicationFence !== fence) return current;
			const endpointMatches = endpoint && (endpointGeneration === undefined || endpoint.generation === endpointGeneration);
			return nextRecord(current, {
	...
	async notify(sessionId: string, body: string, actionId?: string): Promise<SlackConversation> {
		...
		let conversation = usedExistingRoot ? existing.record : await this.postRoot(sessionId, body);
		...
		if (!actionId) {
			await this.#postDurable(`notification:${sessionId}:${this.#randomId()}`, sessionId, conversation.endpointGeneration ?? 0, {
		...
		const intent = await this.store.transact(key, current => {
			if (!current || !acceptsSlackInbound(current, conversation.rootTs!, current.endpointGeneration ?? -1)) return current;
			const now = this.#now();
			if (current.outboundActionId && current.outboundActionId !== actionId) return current;
			if (current.outboundActionId === actionId) {
				if (current.outboundActionOwner === this.#publicationOwnerId || !this.#leaseExpired(current.outboundActionLeaseExpiresAt, now)) return current;
				actionClaimed = true;
				return nextRecord(current, { outboundActionOwner: this.#publicationOwnerId, outboundActionLeaseExpiresAt: now + this.#publicationLeaseMs, outboundActionFence: (current.outboundActionFence ?? 0) + 1, updatedAt: now });
			}
			actionClaimed = true;
			return nextRecord(current, {
		...
		try {
			await this.#withActionLease(key, clientMsgId, fence, async () => {
				published = await this.#postDurable(`action:${sessionId}:${actionId}`, sessionId, conversation.endpointGeneration ?? 0, { channel: conversation.channelId, threadTs: conversation.rootTs, text: body, clientMsgId });
			});
		} catch (error) {
			if (this.#isUncertainPostFailure(error)) {
			...
			if (!published) {
				if (!this.#isUncertainPostFailure(error)) {
					await this.store.transact(key, current => current?.outboundActionClientMsgId === clientMsgId && current.outboundActionOwner === this.#publicationOwnerId && current.outboundActionFence === fence
						? nextRecord(current, { outboundActionId: undefined, outboundActionClientMsgId: undefined, outboundActionOwner: undefined, outboundActionLeaseExpiresAt: undefined, updatedAt: this.#now() })
						: current);
				}
				throw error;
			}
		...
		const active = await this.store.transact(key, current =>
			current?.outboundActionClientMsgId === clientMsgId && current.outboundActionOwner === this.#publicationOwnerId && current.outboundActionFence === fence
				? nextRecord(current, { pendingActionId: actionId, outboundActionId: undefined, outboundActionClientMsgId: undefined, outboundActionOwner: undefined, outboundActionLeaseExpiresAt: undefined, updatedAt: this.#now() })
				: current,
		);
	...
	async postCommandResult(sessionId: string, content: string): Promise<boolean> {
		...
		if (!found || found.record.state !== "active" || !found.record.rootTs) return false;
		await this.#postDurable(`command-result:${sessionId}:${this.#randomId()}`, sessionId, found.record.endpointGeneration ?? 0, {
	...
	async close(sessionId: string, marker = "Session closed."): Promise<boolean> {
		const found = await this.findSession(sessionId, true);
		if (!found || !found.record.rootTs || found.record.state !== "active") return false;
		await this.#postDurable(`close-marker:${sessionId}:${found.record.clientMsgId ?? found.record.rootTs}`, sessionId, found.record.endpointGeneration ?? 0, {
			channel: found.record.channelId,
			threadTs: found.record.rootTs,
			text: marker,
			clientMsgId: this.#randomId(),
		...
		await this.store.transact(found.key, current =>
			current ? nextRecord(current, { state: "closed_marker", pendingActionId: undefined, updatedAt: this.#now() }) : current,
		);
	...
	async resume(sessionId: string, body: string, endpointGeneration?: number): Promise<SlackConversation> {
		...
		if (previous?.record.state === "active" && endpointGeneration !== undefined && previous.record.endpointGeneration === endpointGeneration) return previous.record;
		...
		if (previous) {
			await this.store.transact(previous.key, current =>
				current ? nextRecord(current, { state: "resumed_root", pendingActionId: undefined, updatedAt: this.#now() }) : current,
			);
		}
		return await this.postRoot(sessionId, body, endpointGeneration);
	}

	async #claimInbound(inbound: { eventId: string; eventContext?: string; teamId: string; channelId: string; rootTs: string; event: SlackEvent }): Promise<{ key: string; endpoint: SlackEndpoint; sessionId: string; receipt: SlackInboundDispatchReceipt } | undefined> {
		...
		const matched = Object.entries(document.conversations)
			...
			.filter(({ candidate }) => candidate.teamId === inbound.teamId && candidate.channelId === inbound.channelId && candidate.rootTs === inbound.rootTs)
			.sort((left, right) => (right.candidate.endpointGeneration ?? -1) - (left.candidate.endpointGeneration ?? -1) || right.candidate.generation - left.candidate.generation || right.candidate.updatedAt - left.candidate.updatedAt)[0];
		if (!matched) return undefined;
		const { mappingKey: key, candidate: record } = matched;
		if (!record.sessionId || !record.endpointGeneration || !acceptsSlackInbound(record, inbound.rootTs, record.endpointGeneration)) return undefined;
		...
		const payload: SlackInboundEffectPayload | undefined = command
			...
			: record.pendingActionId ? { type: "reply", id: record.pendingActionId, answer: inboundText ?? "", idempotencyKey, routing } : undefined;
		if (!payload) return undefined;
		await this.#rescheduleAfterEffectTransition(this.#journal.enqueue({ id: effectId, kind: command ? "sdk.inbound.command" : "sdk.inbound.reply", transport: "slack", sessionId: record.sessionId, endpointGeneration: endpoint.generation, payload }));
		let sessionId: string | undefined;
		let receipt: SlackInboundDispatchReceipt | undefined;
		await this.store.transact(key, current => {
			if (!current || !current.sessionId || !acceptsSlackInbound(current, inbound.rootTs, endpoint.generation)) return current;
			const existing = (current.inboundDispatches ?? []).find(candidate => candidate.eventId === inbound.eventId || candidate.interactionId === interactionId || candidate.retryKey === retryKey || (inbound.eventContext !== undefined && candidate.eventContext === inbound.eventContext));
			if (existing) {
				sessionId = current.sessionId;
				receipt = existing;
				return current;
			}
			if (current.seenEventIds.includes(inbound.eventId) || current.seenInteractionIds.includes(interactionId) || current.seenRetryKeys.includes(retryKey) || (inbound.eventContext !== undefined && current.seenContextIds.includes(inbound.eventContext)) || (!command && !current.pendingActionId)) return current;
			sessionId = current.sessionId;
			receipt = { key: `${inbound.eventId}:${interactionId}`, eventId: inbound.eventId, interactionId, retryKey, eventContext: inbound.eventContext, kind: command ? "command" : "action", ...(command ? {} : { actionId: current.pendingActionId }), endpointGeneration: endpoint.generation, effectId, idempotencyKey };
			return nextRecord(current, { inboundDispatches: [...(current.inboundDispatches ?? []), receipt], updatedAt: this.#now() });
		});
		return sessionId && receipt ? { key, endpoint, sessionId, receipt } : undefined;
	}

	async #dispatchInbound(claim: { key: string; endpoint: SlackEndpoint; sessionId: string; receipt: SlackInboundDispatchReceipt }): Promise<boolean> {
		const current = await this.store.read(claim.key);
		if (!current || !acceptsSlackInbound(current, current.rootTs ?? "", claim.endpoint.generation) || claim.receipt.endpointGeneration !== claim.endpoint.generation || !(current.inboundDispatches ?? []).some(receipt => receipt.key === claim.receipt.key)) return false;
		return await this.#dispatchEffect(claim, claim.receipt.effectId);
	}

	async #dispatchEffect(claim: { key: string; endpoint: SlackEndpoint; sessionId: string; receipt: SlackInboundDispatchReceipt }, effectId: string): Promise<boolean> {
		const effect = await this.#rescheduleAfterEffectTransition(this.#journal.claim<{ type: "reply"; id: string; answer: string; idempotencyKey: string } | { type: "command"; content: string; idempotencyKey: string }>(effectId, this.#publicationOwnerId, Math.max(this.#publicationLeaseMs, 100)));
		if (!effect) return false;
		const lease: ChatEffectLease = { owner: this.#publicationOwnerId, epoch: effect.epoch };
		if (!(await this.#rescheduleAfterEffectTransition(this.#journal.renew(effect.id, lease, Math.max(this.#publicationLeaseMs, 100))))) return false;
		try {
			if (effect.payload.type === "command") {
				...
				const accepted = await this.#withEffectLease(effect.id, lease, async () => {
					...
					return await (this.options.onCommand?.(claim.sessionId, payload.content, claim.endpoint, payload.idempotencyKey) ?? Promise.resolve(false));
				});
				if (!(await this.#inboundEffectCurrent(claim, effect.id))) return false;
				const recorded = await this.#journal.record(effect.id, lease, "terminal", { status: accepted ? "accepted" : "rejected" });
			...
			const recorded = await this.#rescheduleAfterEffectTransition(this.#journal.record(effect.id, lease, state, { status: state }));
	...
	async #inboundEffectCurrent(claim: { key: string; endpoint: SlackEndpoint; sessionId: string; receipt: SlackInboundDispatchReceipt }, effectId: string): Promise<boolean> {
		...
		return !!current &&
			...
			(current.inboundDispatches ?? []).some(receipt => receipt.key === claim.receipt.key && receipt.effectId === effectId);
	}


	async #withEffectLease<T>(id: string, lease: ChatEffectLease, operation: () => Promise<T>): Promise<T> {
		return await this.#withRenewal(operation, async () => {
			if (!(await this.#rescheduleAfterEffectTransition(this.#journal.renew(id, lease, Math.max(this.#publicationLeaseMs, 100))))) throw new Error("Slack effect lease renewal failed");
		});
	}
	...
	async #drainPendingDispatches(): Promise<void> {
		...
		for (const effect of await this.#journal.list()) {
			if (mappedEffectIds.has(effect.id) || effect.transport !== "slack" || effect.state === "terminal" || !effect.sessionId || (effect.kind !== "sdk.inbound.command" && effect.kind !== "sdk.inbound.reply")) continue;
			...
			if (!endpoint || endpoint.generation !== adopted.receipt.endpointGeneration || !(await this.#inboundEffectCurrent({ key: adopted.key, endpoint, sessionId: adopted.sessionId, receipt: adopted.receipt }, effect.id))) {
				await this.#terminalizeRejectedInbound(effect.id);
				continue;
			}
			await this.#dispatchInbound({ key: adopted.key, endpoint, sessionId: adopted.sessionId, receipt: adopted.receipt });
		}
	}

	async #adoptOrphanInbound(effect: ChatEffect<SlackInboundEffectPayload>): Promise<{ key: string; sessionId: string; receipt: SlackInboundDispatchReceipt } | undefined> {
		const payload = effect.payload;
		...
		const matches = Object.entries((await this.store.load()).conversations)
			.filter(([, record]) => record.teamId === routing.teamId && record.channelId === routing.channelId && record.rootTs === routing.rootTs);
		if (matches.length !== 1) {
			await this.#terminalizeRejectedInbound(effect.id);
			return undefined;
		}
		...
		let receipt: SlackInboundDispatchReceipt | undefined;
		await this.store.transact(key, current => {
			if (!validPayload || !current || !effect.sessionId || current.sessionId !== effect.sessionId || !acceptsSlackInbound(current, routing.rootTs, effect.endpointGeneration)) return current;
			const existing = (current.inboundDispatches ?? []).find(candidate => candidate.eventId === routing.eventId || candidate.interactionId === routing.interactionId || candidate.retryKey === routing.retryKey || (routing.eventContext !== undefined && candidate.eventContext === routing.eventContext));
			if (existing) {
				if (existing.effectId === effect.id && existing.idempotencyKey === payload.idempotencyKey && existing.endpointGeneration === effect.endpointGeneration && existing.eventId === routing.eventId && existing.interactionId === routing.interactionId && existing.retryKey === routing.retryKey && existing.eventContext === routing.eventContext && existing.kind === routing.kind && existing.actionId === routing.actionId) receipt = existing;
				return current;
			}
			if (current.seenEventIds.includes(routing.eventId) || current.seenInteractionIds.includes(routing.interactionId) || current.seenRetryKeys.includes(routing.retryKey) || (routing.eventContext !== undefined && current.seenContextIds.includes(routing.eventContext)) || (routing.kind === "action" && current.pendingActionId !== routing.actionId)) return current;
			receipt = { key: `${routing.eventId}:${routing.interactionId}`, eventId: routing.eventId, interactionId: routing.interactionId, retryKey: routing.retryKey, eventContext: routing.eventContext, kind: routing.kind, ...(routing.kind === "action" ? { actionId: routing.actionId } : {}), endpointGeneration: effect.endpointGeneration, effectId: effect.id, idempotencyKey: payload.idempotencyKey };
			return nextRecord(current, { inboundDispatches: [...(current.inboundDispatches ?? []), receipt], updatedAt: this.#now() });
		});
	...
	async #finishDispatch(claim: { key: string; endpoint: SlackEndpoint; receipt: SlackInboundDispatchReceipt }, state: "terminal" | "uncertain"): Promise<void> {
		await this.store.transact(claim.key, current => {
			...
			return nextRecord(current, {
				pendingActionId: found.kind === "action" && current.pendingActionId === found.actionId ? undefined : current.pendingActionId,
				...
				seenContextIds: found.eventContext === undefined ? current.seenContextIds : [...current.seenContextIds, found.eventContext],
				inboundDispatches: state === "terminal" ? (current.inboundDispatches ?? []).filter(receipt => receipt.key !== found.key) : current.inboundDispatches,
				updatedAt: this.#now(),
			});
		});
	}

	async #releaseDispatch(claim: { key: string; endpoint: SlackEndpoint; receipt: SlackInboundDispatchReceipt }): Promise<void> {
		await this.store.transact(claim.key, current => {
			if (!current || !acceptsSlackInbound(current, current.rootTs ?? "", claim.endpoint.generation)) return current;
			const found = (current.inboundDispatches ?? []).find(receipt => receipt.key === claim.receipt.key);
			return found ? nextRecord(current, { inboundDispatches: (current.inboundDispatches ?? []).filter(receipt => receipt.key !== found.key), updatedAt: this.#now() }) : current;
		});
	}
	...
	async #reconcileTerminalProviderReceipts(): Promise<void> {
		for (const effect of await this.#journal.list()) {
			try {
				if (effect.kind !== "provider-post" || effect.transport !== "slack" || effect.state !== "terminal") continue;
				const payload = effect.payload as { channel?: unknown; threadTs?: unknown; clientMsgId?: unknown };
				const receipt = effect.receipt;
				if (receipt?.provider !== "slack" || receipt.status !== "posted" || typeof receipt.timestamp !== "string" || typeof payload.clientMsgId !== "string") continue;
				const normalized = { channel: payload.channel, threadTs: payload.threadTs, clientMsgId: payload.clientMsgId };
	...
	async #activateReconciledRoot(effect: ChatEffect, payload: { channel?: unknown; clientMsgId: string }, rootTs: string): Promise<void> {
		if (!effect.sessionId || typeof payload.channel !== "string" || payload.channel !== this.options.channelId) return;
		...
		await this.store.transact(this.#intentKey(effect.sessionId), current =>
			...
			(effect.endpointGeneration === 0 || current.endpointGeneration === undefined || current.endpointGeneration === effect.endpointGeneration)
				? nextRecord(current, {
					state: "active",
					rootTs,
	...
	async #activateReconciledAction(effect: ChatEffect, payload: { channel?: unknown; threadTs?: unknown; clientMsgId: string }): Promise<void> {
	...
	async #drainProviderEffects(): Promise<boolean> {
		...
		for (const effect of await this.#journal.list()) {
			try {
				...
				const payload = effect.payload as { channel?: unknown; text?: unknown; threadTs?: unknown; clientMsgId?: unknown };
				if (typeof payload.channel !== "string" || typeof payload.text !== "string" || typeof payload.clientMsgId !== "string" || (payload.threadTs !== undefined && typeof payload.threadTs !== "string")) continue;
				await this.#postDurable(effect.id, effect.sessionId, effect.endpointGeneration, { channel: payload.channel, text: payload.text, ...(typeof payload.threadTs === "string" ? { threadTs: payload.threadTs } : {}), clientMsgId: payload.clientMsgId });
			} catch {
				failed = true;
				await this.#recordRecoveryFailure(effect);
			}
	...
	async #recoverLeasedEffects(): Promise<void> {
		...
		let failed = false;
		try {
			try { await this.#reconcileTerminalProviderReceipts(); } catch { failed = true; }
	...
	async #providerEffectCurrent(effect: ChatEffect): Promise<boolean> {
		...
		const endpoint = await this.#resolveEndpoint(effect.sessionId);
		if (!endpoint || (effect.endpointGeneration !== 0 && endpoint.generation !== effect.endpointGeneration)) return false;
		...
		if (typeof threadTs === "string") {
			return records.some(record => record.sessionId === effect.sessionId && acceptsSlackInbound(record, threadTs, effect.endpointGeneration));
		}
		return records.some(record => record.sessionId === effect.sessionId && (record.state === "posting_root" || (record.state === "active" && record.endpointGeneration === effect.endpointGeneration)));
	}

	async #postDurable(id: string, sessionId: string, endpointGeneration: number, payload: { channel: string; text: string; threadTs?: string; clientMsgId: string }): Promise<SlackPostedMessage> {
		const initial = await this.#rescheduleAfterEffectTransition(this.#journal.enqueue({ id, kind: "provider-post", transport: "slack", sessionId, endpointGeneration, payload }));
		const fromReceipt = (effect: typeof initial): SlackPostedMessage | undefined => {
			if (effect.state !== "terminal" || effect.receipt?.status !== "posted" || !effect.receipt.channelId || !effect.receipt.timestamp) return undefined;
			return { channel: effect.receipt.channelId, ts: effect.receipt.timestamp, client_msg_id: effect.receipt.messageId };
		};
		...
		for (let attempt = 0; attempt < 100 && !effect; attempt++) {
			effect = await this.#rescheduleAfterEffectTransition(this.#journal.claim<typeof payload>(id, this.#publicationOwnerId, Math.max(this.#publicationLeaseMs, 100)));
			if (effect) break;
		...
		const lease: ChatEffectLease = { owner: this.#publicationOwnerId, epoch: effect.epoch };
		if (!(await this.#rescheduleAfterEffectTransition(this.#journal.renew(id, lease, Math.max(this.#publicationLeaseMs, 100))))) throw new Error("Slack provider effect lease renewal failed");
		try {
			const posted = await this.#withEffectLease(id, lease, async () => {
				...
				const found = await this.options.provider.findMessageByClientMsgId({ channel: effect!.payload.channel, threadTs: effect!.payload.threadTs, clientMsgId: effect!.payload.clientMsgId });
			...
			if (!(await this.#providerEffectCurrent(effect))) throw new SlackStaleEffectError();
			if (!(await this.#journal.record(id, lease, "terminal", { provider: "slack", channelId: posted.channel, timestamp: posted.ts, messageId: posted.client_msg_id ?? posted.ts, status: "posted" }))) throw new Error("Slack provider effect lease expired before commit");
			...
			if (this.#isUncertainPostFailure(error)) {
				try {
					const reconciled = await this.#withEffectLease(id, lease, async () => {
						if (!(await this.#providerEffectCurrent(effect!))) throw new SlackStaleEffectError();
						return await this.options.provider.findMessageByClientMsgId({ channel: effect!.payload.channel, threadTs: effect!.payload.threadTs, clientMsgId: effect!.payload.clientMsgId });
					});
					if (reconciled) {
						if (!(await this.#providerEffectCurrent(effect))) throw new SlackStaleEffectError();
						if (!(await this.#journal.record(id, lease, "terminal", { provider: "slack", channelId: reconciled.channel, timestamp: reconciled.ts, messageId: reconciled.client_msg_id ?? reconciled.ts, status: "posted" }))) throw new Error("Slack provider effect lease expired before commit");
						return reconciled;
					}
			...
			await this.#rescheduleAfterEffectTransition(this.#journal.record(id, lease, this.#isUncertainPostFailure(error) ? "uncertain" : "terminal", { provider: "slack", status: this.#isUncertainPostFailure(error) ? "uncertain" : "failed" }));
			throw error;
		}
	}
	...
	async #withRootLease<T>(key: string, clientMsgId: string, fence: number, operation: () => Promise<T>): Promise<T> {
		return await this.#withPublicationLease(operation, async () => {
			...
			await this.store.transact(key, current => {
				if (!current || current.clientMsgId !== clientMsgId || current.rootPublicationOwner !== this.#publicationOwnerId || current.rootPublicationFence !== fence) return current;
				renewed = true;
				return nextRecord(current, { rootPublicationLeaseExpiresAt: this.#now() + this.#publicationLeaseMs, updatedAt: this.#now() });
			});
	...
	async #withActionLease<T>(key: string, clientMsgId: string, fence: number, operation: () => Promise<T>): Promise<T> {
		return await this.#withPublicationLease(operation, async () => {
			let renewed = false;
			await this.store.transact(key, current => {
				if (!current || current.outboundActionClientMsgId !== clientMsgId || current.outboundActionOwner !== this.#publicationOwnerId || current.outboundActionFence !== fence) return current;
				renewed = true;
				return nextRecord(current, { outboundActionLeaseExpiresAt: this.#now() + this.#publicationLeaseMs, updatedAt: this.#now() });
			});
	...
	async #withRenewal<T>(operation: () => Promise<T>, renew: () => Promise<void>): Promise<T> {
		await renew();
		let failure: unknown;
		let renewing = Promise.resolve();
		const timer = setInterval(() => {
			renewing = renewing.then(renew).catch(error => { failure ??= error; });
	...
	async #waitForRoot(key: string, sessionId: string, body: string, endpointGeneration?: number): Promise<SlackConversation> {
		for (let attempt = 0; attempt < 100; attempt++) {
			await Bun.sleep(10);
			const current = await this.store.read(key);
			if (current?.state === "active") return current;
			if (current?.state === "error" || (current?.state === "posting_root" && this.#leaseExpired(current.rootPublicationLeaseExpiresAt, this.#now()))) {
				return await this.postRoot(sessionId, body, endpointGeneration);
			}
		}
	...
	async #waitForAction(key: string, sessionId: string, body: string, actionId: string): Promise<SlackConversation> {
		for (let attempt = 0; attempt < 100; attempt++) {
			...
			if (current?.outboundActionId === actionId && this.#leaseExpired(current.outboundActionLeaseExpiresAt, this.#now())) {
	...
	private async findSession(sessionId: string, includeInactive: boolean): Promise<{ key: string; record: SlackConversation } | undefined> {
		const document = await this.store.load();
		return Object.entries(document.conversations)
			.map(([key, record]) => ({ key, record }))
			.filter(entry =>
				entry.record.sessionId === sessionId &&
				(includeInactive || entry.record.state === "active" || entry.record.state === "posting_root" || entry.record.state === "error"),
			)
			.sort((left, right) => (right.record.endpointGeneration ?? -1) - (left.record.endpointGeneration ?? -1) || right.record.generation - left.record.generation || right.record.updatedAt - left.record.updatedAt)[0];
	}
	#intentKey(sessionId: string): string {
		return slackConversationKey({ teamId: this.options.teamId, channelId: this.options.channelId, rootTs: `intent:${sessionId}` });
	}
	...
	#isUncertainPostFailure(error: unknown): boolean {
		return error instanceof SlackProviderError && error.operation === "chat.postMessage" && error.code === "connection";
	}
	#isDefiniteSdkPreSendFailure(error: unknown): boolean {
		return error instanceof SlackEndpointBindingError || (error instanceof SdkClientError && error.code === "connection_closed");
	}
}
