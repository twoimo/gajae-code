...
### Fixed
- Composer queue submissions (`Alt+Q` / `app.message.queue`) now force one-at-a-time follow-up delivery for those queued prompts, so they do not collapse into one batched model call even when the broader follow-up mode is set to `all`.
