...
describe("app-server session export redaction", () => {
	it("rejects relative traversal and nonexistent absolute paths before opening sessions", async () => {
		...
		await expect(host.sessionRename({ sessionPath: "../../etc/passwd", title: "Nope" })).rejects.toThrow(
			"absolute local .jsonl",
		);
		await expect(
			host.sessionExport({ sessionPath: path.join(os.tmpdir(), "gjc-missing-session.jsonl"), format: "json" }),
				...
				content: [
					{
						...
						id: "tc1",
						name: "cfg",
						arguments: { Api_Key: "raw-mixed-case", nested: [{ PASSWORD: "raw-upper" }] },
					},
				],
			},
			"m1",
				...
				content: [
					{
						type: "toolCall",
						id: "tc1",
						name: "run-token=tool-name-secret",
						arguments: { url: "ws://x?token=abc", next: 1 },
					},
				],
			},
			"m2",
				...
				content: [
					{
						type: "toolCall",
						id: "tc1",
						name: { "sk-objnamekey1234567": "Bearer objname-raw-value" },
						arguments: {},
					},
				],
			},
	...
	it("renames a valid absolute tmp .jsonl session", async () => {
		...
		await fs.writeFile(sessionPath, `${JSON.stringify(header)}\n`);
		const host = new AgentSessionHost();
		await expect(host.sessionRename({ sessionPath, title: "Renamed" })).resolves.toMatchObject({
			ok: true,
			title: "Renamed",
		});
	});
	...
	it("redacts common secrets by default and preserves raw content when disabled", async () => {
		...
		const secrets =
			"sk-abcdefgh12345678 Bearer bearer-secret AKIA1234567890ABCDEF ghp_abcdefghijklmnopqrstuvwxyz xoxb-1234567890-secret";
		const message = messageEntry(
			{
				...
				content: [
					{ type: "text", text: `token ${secrets}` },
					{
						...
						id: "tc1",
						name: "read",
						arguments: { authorization: "Bearer nested", config: { api_key: "v", password: { deep: "raw" } } },
					},
				],
			},
			"m1",
		...
		const host = new AgentSessionHost();
		const redacted = (await host.sessionExport({ sessionPath, format: "json" })) as {
			content: string;
			provenance: { redacted: boolean };
		};
		const raw = (await host.sessionExport({ sessionPath, format: "json", redact: false })) as {
			content: string;
			provenance: { redacted: boolean };
		};
