...
function selector(model: Model): string { return `${model.provider}/${model.id}`; }

function failingStream(model: Model): AssistantMessageEventStream {
	const stream = new AssistantMessageEventStream();
	queueMicrotask(() => {
		const message: AssistantMessage & { transportFailure: { kind: "transport"; status: number } } = { role: "assistant", content: [], api: model.api, provider: model.provider, model: model.id, usage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, totalTokens: 0, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 } }, stopReason: "error", errorMessage: "rate limit exceeded", errorStatus: 429, timestamp: Date.now(), transportFailure: { kind: "transport", status: 429 } };
...
describe("AgentSession managed fallback cancellation completion", () => {
	...
	afterEach(async () => { await session?.dispose(); authStorage.close(); tempDir.removeSync(); vi.restoreAllMocks(); });
	...
	it("preserves an accepted retry completion when a subscriber aborts after message_end", async () => {
		...
		expect(agentEnds[0]).not.toMatchObject({ stopReason: "cancelled" });
		expect(events.some(event => event.type === "agent_end" && event.stopReason === "cancelled")).toBe(false);
		expect(session!.messages.at(-1)).toMatchObject({ role: "assistant", content: [{ type: "text", text: "accepted retry output" }] });
		expect(session!.isStreaming).toBe(false);
	});
