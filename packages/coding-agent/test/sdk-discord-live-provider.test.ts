...
import { DiscordLiveProvider, type DiscordGatewaySocket, type DiscordTimer } from "../src/sdk/bus/discord-live-provider";
...
class FakeSocket implements DiscordGatewaySocket {
	...
	message(frame: Record<string, unknown>): void { this.emit("message", new MessageEvent("message", { data: JSON.stringify(frame) })); }
}

function response(body: unknown, status = 200): Response { return new Response(JSON.stringify(body), { status, headers: { "Content-Type": "application/json" } }); }

function provider(requests: Array<{ path: string; init: RequestInit }>, sockets: FakeSocket[], sleeps: number[] = [], heartbeats: Array<() => void> = []): DiscordLiveProvider {
	return new DiscordLiveProvider({
		...
		fetchImpl: async (input, init) => {
			...
			if (path.includes("/threads/active")) return response({ threads: [{ id: "thread", parent_id: "parent", thread_metadata: { archived: false } }] });
			...
			if (path.endsWith("/channels/parent/messages/starter/threads")) return response({ id: "thread", parent_id: "parent", thread_metadata: { archived: false } });
		...
		WebSocketImpl: url => { expect(url).toBe("wss://gateway.test/?v=10&encoding=json"); const socket = new FakeSocket(); sockets.push(socket); return socket; },
		sleep: async milliseconds => { sleeps.push(milliseconds); },
		setIntervalImpl: callback => { heartbeats.push(callback); return { cancel() {} }; },
		setTimeoutImpl: callback => { callback(); return { cancel() {} }; },
	});
}
...
describe("DiscordLiveProvider protocol", () => {
	test("creates a generic-text-parent thread from a durable nonce starter message", async () => {
		const requests: Array<{ path: string; init: RequestInit }> = []; const sockets: FakeSocket[] = [];
		...
		const thread = requests.find(request => request.path === "https://discord.test/api/channels/parent/messages/starter/threads")!;
	...
	test("reconciles an accepted uncertain create through its parent nonce without a duplicate thread", async () => {
		...
		const live = new DiscordLiveProvider({
			applicationId: "app", botToken: "discord-secret-token", apiBaseUrl: "https://discord.test/api",
			fetchImpl: async input => {
				const path = String(input);
				if (path.endsWith("/channels/parent/messages?limit=100")) {
					return response(threadCreated ? [{ id: "starter", content: "<!-- gjc-thread-nonce:nonce -->", thread: { id: "public-thread", thread_metadata: { archived: false } } }] : []);
				}
				if (path.endsWith("/channels/parent/messages")) { starterMessages++; return response({ id: "starter" }); }
		...
		await expect(live.createThread({ guildId: "guild", parentId: "parent", name: "Session", nonce: "nonce" })).rejects.toThrow("connection lost");
		expect(await live.createThread({ guildId: "guild", parentId: "parent", name: "Session", nonce: "nonce" })).toMatchObject({ id: "public-thread", parentId: "parent" });
	...
	test("serializes Discord select controls and maps selected gateway values", async () => {
		const requests: Array<{ path: string; init: RequestInit }> = []; const sockets: FakeSocket[] = []; const events: DiscordInboundEvent[] = [];
		const live = provider(requests, sockets);
		await live.postMessage({
			threadId: "thread", content: "Choose",
			components: [{ type: 1, components: [{ type: 3, customId: "gjc:4:ask", options: [{ label: "Yes", value: "yes" }] }] }],
		});
		const payload = JSON.parse(String(requests[0]?.init.body)) as { components: Array<{ components: Array<{ custom_id: string; options: Array<{ value: string }> }> }> };
		expect(payload.components[0]?.components[0]).toMatchObject({ custom_id: "gjc:4:ask", options: [{ value: "yes" }] });
		await live.start(async event => { events.push(event); });
		const socket = sockets[0]!;
		socket.message({ op: 10, d: { heartbeat_interval: 1 } });
		socket.message({ op: 0, t: "INTERACTION_CREATE", s: 5, d: { id: "interaction", token: "interaction-token", guild_id: "guild", channel_id: "thread", channel: { parent_id: "parent" }, member: { user: { id: "member" } }, data: { custom_id: "gjc:4:ask", values: ["yes"] } } });
		await Promise.resolve();
		expect(events[0]?.interaction).toEqual({ id: "interaction", token: "interaction-token", customId: "gjc:4:ask", value: "yes" });
		await live.stop();
	});
	...
	test("defers accepted components through the unauthenticated interaction callback without leaking credentials", async () => {
		...
		expect(callback.path).toBe("https://discord.test/api/interactions/interaction/interaction-callback-token/callback");
	...
	test("sends stable message nonces with Discord provider enforcement", async () => {
		...
		expect(JSON.parse(String(post.init.body))).toEqual({ content: "durable", nonce: "gjc-stable-nonce", enforce_nonce: true });
	});
	...
	test("uses provider-enforced stable nonces after more than one hundred intervening messages", async () => {
		...
		const live = new DiscordLiveProvider({
			...
			fetchImpl: async (input, init) => {
				...
				const existing = payload.nonce === undefined || !payload.enforce_nonce ? undefined : accepted.get(payload.nonce);
		...
		for (let index = 0; index < 101; index++) await live.postMessage({ threadId: "thread", content: `churn-${index}` });
	...
	test("finds an accepted post by its stable nonce for journal receipt reconstruction", async () => {
		...
		expect(await live.findMessageByNonce({ threadId: "thread", nonce: "gjc-nonce" })).toEqual({ id: "accepted-message" });
	...
	test("identifies, heartbeats, maps inbound events, reconnects, and stops cleanly", async () => {
		const requests: Array<{ path: string; init: RequestInit }> = []; const sockets: FakeSocket[] = []; const events: string[] = [];
		const live = provider(requests, sockets);
		await live.start(async event => { events.push(event.id); });
		const socket = sockets[0]!;
		socket.message({ op: 10, d: { heartbeat_interval: 1 } });
		expect(socket.sent.map(value => JSON.parse(value))).toContainEqual(expect.objectContaining({ op: 2, d: expect.objectContaining({ token: "discord-secret-token" }) }));
		socket.message({ op: 0, t: "MESSAGE_CREATE", s: 4, d: { id: "message", guild_id: "guild", channel_id: "thread", thread: { parent_id: "parent" }, author: { id: "member", bot: false }, content: "reply" } });
		socket.message({ op: 0, t: "INTERACTION_CREATE", s: 5, d: { id: "interaction", token: "interaction-token", guild_id: "guild", channel_id: "thread", channel: { parent_id: "parent" }, member: { user: { id: "member" } }, data: { custom_id: "gjc:1:ask", value: "yes" } } });
		await Promise.resolve();
	...
	test("bounds rate-limit retries without leaking its token in errors", async () => {
		...
		const limited = new DiscordLiveProvider({ applicationId: "app", botToken: "discord-secret-token", fetchImpl: async () => { calls++; return response({ retry_after: 0.01 }, 429); }, sleep: async milliseconds => { sleeps.push(milliseconds); } });
		let error = "";
		try { await limited.postMessage({ threadId: "thread", content: "x" }); } catch (caught) { error = caught instanceof Error ? caught.message : String(caught); }
		expect(error).toBe("Discord API rate limit retry exhausted");
		expect(error).not.toContain("discord-secret-token");
		expect(calls).toBe(3); expect(sleeps).toEqual([10, 10]);
	});
	...
	test("resets stopped state after startup failure so a later start can connect", async () => {
		...
		const live = new DiscordLiveProvider({
			...
			fetchImpl: async input => {
				...
				return String(input).endsWith("/users/@me") ? response({ id: "bot" }) : response({ url: "wss://gateway.test" });
			},
			WebSocketImpl: () => {
	...
	test("non-resumable invalid session clears resume state and waits before identifying", async () => {
		...
		const live = new DiscordLiveProvider({
			...
			botToken: "discord-secret-token",
			apiBaseUrl: "https://discord.test/api",
			fetchImpl: async input => String(input).endsWith("/users/@me") ? response({ id: "bot" }) : response({ url: "wss://gateway.test" }),
			WebSocketImpl: () => {
				const socket = new FakeSocket();
				sockets.push(socket);
				return socket;
		...
		first.message({ op: 0, t: "READY", s: 7, d: { session_id: "resume-me", resume_gateway_url: "wss://resume.test" } });
	...
	test("resumable invalid session retains resume state and resumes after the retry delay", async () => {
		...
		const second = sockets[1]!;
		second.message({ op: 10, d: { heartbeat_interval: 1 } });
		expect(second.sent.map(value => JSON.parse(value))).toContainEqual({ op: 6, d: { token: "discord-secret-token", session_id: "resume-me", seq: 7 } });
		await live.stop();
	});
