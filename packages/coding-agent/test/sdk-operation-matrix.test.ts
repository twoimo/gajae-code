...
import { ADAPTERS, OPERATIONS, type AdapterDisposition } from "../src/sdk/protocol/operation-registry.js";

type InventoryRow = { sourceId: string; sourceKind: string; sdkId?: string; adapterMappings: Record<string, AdapterDisposition>; testIds: string[] };
const inventoryPath = new URL("../src/sdk/protocol/operation-inventory.generated.json", import.meta.url);
const allowedDispositions = new Set<AdapterDisposition>(["native_alias", "generic_safe", "machine_only", "provider_only", "prohibited"]);

const expectedDispositions: Record<string, Record<string, AdapterDisposition>> = {
	G02: { telegram: "prohibited", discord: "prohibited", slack: "prohibited", mcp: "prohibited", acp: "machine_only", daemonCli: "machine_only" },
	C25: { telegram: "prohibited", discord: "prohibited", slack: "prohibited", mcp: "generic_safe", acp: "generic_safe", daemonCli: "generic_safe" },
	C26: { telegram: "prohibited", discord: "prohibited", slack: "prohibited", mcp: "generic_safe", acp: "generic_safe", daemonCli: "generic_safe" },
	C52: { telegram: "prohibited", discord: "prohibited", slack: "prohibited", mcp: "generic_safe", acp: "generic_safe", daemonCli: "generic_safe" },
	C38: { telegram: "prohibited", discord: "prohibited", slack: "prohibited", mcp: "prohibited", acp: "provider_only", daemonCli: "machine_only" },
	C39: { telegram: "prohibited", discord: "prohibited", slack: "prohibited", mcp: "prohibited", acp: "provider_only", daemonCli: "prohibited" },
	C40: { telegram: "prohibited", discord: "prohibited", slack: "prohibited", mcp: "prohibited", acp: "provider_only", daemonCli: "prohibited" },
};
...
describe("SDK operation matrix", () => {
	it("has a complete generated-registry bijection with adapter and test coverage", async () => {
		...
		expect(registryInventory).toHaveLength(90);
		...
		for (const row of registryInventory) {
			...
			for (const disposition of Object.values(row.adapterMappings)) expect(allowedDispositions.has(disposition)).toBe(true);
	...
	it("keeps control errors, query continuity, counts, and the stage-05 adapter partition explicit", () => {
		...
		for (const operation of OPERATIONS.filter(operation => operation.kind === "control")) expect(operation.errorCodes.length).toBeGreaterThan(0);
		for (const operation of OPERATIONS.filter(operation => operation.kind === "query")) expect(operation.continuityClass).toBeDefined();
		for (const [id, disposition] of Object.entries(expectedDispositions)) expect(OPERATIONS.find(operation => operation.id === id)?.adapterDispositions).toEqual(disposition);
	});
	...
	it("gives every generated operation an explicit reviewed Discord and Slack chat disposition", () => {
		for (const transport of ["discord", "slack"] as const) {
			expect(Object.keys(CHAT_OPERATION_POLICY[transport]).sort()).toEqual(OPERATIONS.map(operation => operation.id).sort());
			for (const operation of OPERATIONS) expect(["allowed", "unsupported_on_chat"]).toContain(CHAT_OPERATION_POLICY[transport][operation.id]);
		}
	});
	...
	it("rejects prohibited and unknown chat payloads before SDK send without echoing secrets", async () => {
		for (const transport of ["discord", "slack"] as const) {
			for (const request of [
				...
				let sends = 0;
				const result = await sendAuthorizedChatOperation(transport, request, async () => { sends++; return "sent"; });
