...
import { DiscordNotificationDaemon, type DiscordEndpointBinding, type DiscordNotificationDaemonOptions } from "../src/sdk/bus/discord-daemon";
import { ChatEffectJournal, type ChatEffect, type ChatEffectLease } from "../src/sdk/bus/chat-effect-journal";
...
import type { DiscordInboundEvent, DiscordMessageComponent, DiscordProvider, DiscordThread } from "../src/sdk/bus/discord-provider";
...
class FakeDiscordProvider implements DiscordProvider {
	...
	async createThread(input: { guildId: string; parentId: string; name: string; nonce: string }): Promise<DiscordThread> {
		this.creates++;
		const thread = { id: `thread-${this.creates}`, guildId: input.guildId, parentId: input.parentId, archived: false };
	...
	async postMessage(input: { threadId: string; content: string; nonce?: string; components?: DiscordMessageComponent[] }): Promise<{ id: string }> {
		...
		this.messages.push({ threadId: input.threadId, content: input.content, ...(input.components ? { components: input.components } : {}) });
		...
		if (this.failPostAfterPersist) { this.failPostAfterPersist = false; throw new Error("Discord disconnected after accepting post"); }
		return { id };
	}
...
async function withDaemon(
	...
	try {
		...
		daemon = new DiscordNotificationDaemon({
			agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider,
...
function inbound(threadId: string, id: string, generation = 1): DiscordInboundEvent {
	return {
		...
		threadId,
		authorId: "member",
		interaction: { id: `interaction-${id}`, token: `token-${id}`, customId: actionCustomIds.get(threadId) ?? `gjc:${generation}:ask:00000000-0000-0000-0000-000000000000`, value: "yes" },
...
describe("DiscordNotificationDaemon fake-provider acceptance", () => {
	test("reconciles an uncertain create by nonce instead of creating a second thread", async () => {
		await withDaemon(async (daemon, provider) => {
			...
			await expect(daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "open" })).rejects.toThrow("disconnected");
	...
	test("elects one durable creator while another daemon waits for its mapping", async () => {
		await withDaemon(async (daemon, provider, agentDir) => {
			...
			const other = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, resolveEndpoint: async () => ({ generation: 1, isCurrent: () => true, send: () => {} }) });
	...
	test("restores a durable mapping after daemon restart without creating another thread", async () => {
		await withDaemon(async (daemon, provider, agentDir) => {
			...
			const restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, resolveEndpoint: async () => ({ generation: 2, isCurrent: () => true, send: () => {} }) });
			const restored = await restarted.notify({ sessionId: "session", endpointGeneration: 2, content: "after restart" });
	...
	test("surfaces notification permission and rate-limit failures without leaking a route", async () => {
		await withDaemon(async (daemon, provider) => {
			...
			await expect(daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "open" })).rejects.toThrow("rate limited");
	...
	test("emits a real select component and rejects it after action resolution or restart", async () => {
		await withDaemon(async (daemon, provider, agentDir) => {
			const conversation = await daemon.notify({
				sessionId: "session", endpointGeneration: 4, content: "Choose", actionId: "ask", options: ["Yes", "No"],
			});
			expect(provider.messages[0]?.components).toEqual([{
				type: 1,
				components: [{
					type: 3, customId: expect.stringMatching(/^gjc:4:ask:[0-9a-f-]{36}$/), placeholder: "Choose an option", minValues: 1, maxValues: 1,

					options: [{ label: "Yes", value: "0" }, { label: "No", value: "1" }],
				}],
			}]);
			const restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, resolveEndpoint: async () => ({ generation: 4, isCurrent: () => true, send: () => {} }) });
			await restarted.resolveAction("session", "ask");
			await restarted.handleInbound(inbound(conversation.threadId!, "resolved", 4));
			expect(provider.messages.at(-1)).toEqual({ threadId: conversation.threadId!, content: "This conversation is no longer available." });
		});
	});
	...
	test("rejects /sdk commands from archived and superseded threads", async () => {
		...
		await withDaemon(async (daemon, provider) => {
			const original = await daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "open" });
			await daemon.archive("session");
			await daemon.handleInbound({ ...inbound(original.threadId!, "archived-command"), interaction: undefined, content: "/sdk query Q01" });
			provider.failUnarchive = true;
			const replacement = await daemon.resume("session", 2);
			await daemon.handleInbound({ ...inbound(original.threadId!, "superseded-command"), interaction: undefined, content: "/sdk query Q01" });
		...
		}, { onCommand: async (_sessionId, command) => { commands.push(command); return true; } });
	});
	...
	test("claims a /sdk command before dispatch so redelivery runs it once", async () => {
		...
		await withDaemon(async daemon => {
			...
			const command = { ...inbound(conversation.threadId!, "command-redelivery"), interaction: undefined, content: "/sdk query Q01" };
	...
	test("claims concurrent duplicate interactions durably before one SDK reply", async () => {
		...
		const client = { send: (frame: Record<string, unknown>) => { frames.push(frame); } };
		await withDaemon(async daemon => {
			const conversation = await daemon.notify({ sessionId: "session", endpointGeneration: 4, content: "Choose", actionId: "ask", options: ["Yes"] });
			...
			expect(frames).toMatchObject([{ type: "reply", id: "ask", answer: "yes", idempotencyKey: expect.any(String) }]);
		}, {
	...
	test("defers an interaction before a blocked endpoint resolver completes", async () => {
		...
		await withDaemon(async (daemon, provider) => {
			const conversation = await daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "Choose", actionId: "ask", options: ["Yes"] });
			provider.deferInteraction = async () => { deferred.resolve(); };
			...
			resolveEndpoint: async () => {
				...
				return { generation: 1, isCurrent: () => true, send: frame => { frames.push(frame); } };
			},
		});
	});
	...
	test("waits for an in-flight renewal before posting a provider message", async () => {
		...
		try {
			await withDaemon(async (daemon, provider, agentDir) => {
				...
				vi.spyOn(ChatEffectJournal.prototype, "renew").mockImplementation(async function <TPayload = unknown>(this: ChatEffectJournal, id: string, lease: ChatEffectLease, leaseMs: number): Promise<ChatEffect<TPayload> | undefined> {
					if (blockRenewal) {
						blockRenewal = false;
				...
				const pending = (await new ChatEffectJournal({ agentDir, transport: "discord" }).list()).find(effect => effect.kind === "post-message" && effect.payload && (effect.payload as { content?: string }).content === "blocked");
	...
	test("recovers a create-thread effect at startup after remote creation without another notification", async () => {
		...
		await withDaemon(async (daemon, provider, agentDir) => {
			...
			const restarted = new DiscordNotificationDaemon({
				agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, now: () => now,
				resolveEndpoint: async () => ({ generation: 1, isCurrent: () => true, send: () => {} }),
			});
			await restarted.start();
			const stored = await fs.readFile(path.join(agentDir, "sdk", "daemons", "discord", "conversations.json"), "utf8");
	...
	test("recovers a live transient provider failure without restart, input, or another notification", async () => {
		await withDaemon(async (daemon, provider) => {
			await daemon.start();
			provider.failPost = true;
			await expect(daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "recovered" })).rejects.toThrow("rate limited");
			provider.failPost = false;
			for (let attempt = 0; attempt < 20 && !provider.messages.some(message => message.content === "recovered"); attempt++) await Bun.sleep(25);
	...
	test("reclaims an expired create fence and commits a terminal create-thread receipt at startup", async () => {
		...
		await withDaemon(async (_daemon, provider, agentDir) => {
			...
			const thread = await provider.createThread({ guildId: "guild", parentId: "parent", name: "GJC session", nonce });
			const journal = new ChatEffectJournal({ agentDir, transport: "discord", now: () => now });
			const effectId = "create:session:recovery-nonce";
			await journal.enqueue({ id: effectId, kind: "create-thread", transport: "discord", sessionId: "session", endpointGeneration: 1, payload: { guildId: "guild", parentId: "parent", name: "GJC session", nonce } });
			const leased = await journal.claim(effectId, "crashed-owner", 1);
			expect(leased).toBeDefined();
			await journal.record(effectId, { owner: "crashed-owner", epoch: leased!.epoch }, "terminal", { provider: "discord", threadId: thread.id, status: "created" });
			const store = new ConversationStore<DiscordConversation>({ agentDir, kind: "discord", now: () => now });
			await store.transact("app:guild:parent:creating:session", () => ({ generation: 1, state: "creating", appId: "app", guildId: "guild", parentChannelId: "parent", sessionId: "session", endpointGeneration: 1, createNonce: "recovery-nonce", createOwner: "crashed-owner", createLeaseExpiresAt: 1, updatedAt: 1, seenEventIds: [], seenInteractionIds: [] }));
			const restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, now: () => now, resolveEndpoint: async () => ({ generation: 1, isCurrent: () => true, send: () => {} }) });
			await restarted.start();
			const stored = await fs.readFile(path.join(agentDir, "sdk", "daemons", "discord", "conversations.json"), "utf8");
	...
	test("consumes terminal create intents represented by closed or archived mappings and recreates with a fresh effect", async () => {
		for (const state of ["closed", "archived"] as const) {
			await withDaemon(async (_daemon, provider, agentDir) => {
				...
				await journal.enqueue({ id: "create:session:recovery", kind: "create-thread", transport: "discord", sessionId: "session", endpointGeneration: 1, payload: { guildId: "guild", parentId: "parent", name: "GJC session", nonce } });
				const lease = await journal.claim("create:session:recovery", "crashed-owner", 60_000);
				await journal.record("create:session:recovery", { owner: "crashed-owner", epoch: lease!.epoch }, "terminal", { provider: "discord", threadId: thread.id, status: "created" });
				...
				await store.transact(intentKey, () => ({ generation: 1, state: "creating", appId: "app", guildId: "guild", parentChannelId: "parent", sessionId: "session", endpointGeneration: 1, createNonce: "recovery", updatedAt: 1, seenEventIds: [], seenInteractionIds: [] }));
				const key = `app:guild:parent:${thread.id}`;
				await store.transact(key, () => ({ generation: 1, state, appId: "app", guildId: "guild", parentChannelId: "parent", threadId: thread.id, sessionId: "session", endpointGeneration: 1, updatedAt: 1, seenEventIds: [], seenInteractionIds: [] }));
				const restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, resolveEndpoint: async () => ({ generation: 2, isCurrent: () => true, send: () => {} }) });
				await restarted.start();
				expect((await store.read(key))?.state).toBe(state);
				expect(await store.read(intentKey)).toBeUndefined();
				const replacement = await restarted.notify({ sessionId: "session", endpointGeneration: 2, content: "reopened" });
				expect(replacement.threadId).toBe("thread-2");
				expect(provider.creates).toBe(2);
				expect((await store.read(key))?.state).toBe(state);
	...
	test("does not activate a terminal create receipt when another mapping already owns the session", async () => {
		await withDaemon(async (_daemon, provider, agentDir) => {
			...
			const stale = await provider.createThread({ guildId: "guild", parentId: "parent", name: "GJC session", nonce });
			const journal = new ChatEffectJournal({ agentDir, transport: "discord" });
			await journal.enqueue({ id: "create:session:stale", kind: "create-thread", transport: "discord", sessionId: "session", endpointGeneration: 1, payload: { guildId: "guild", parentId: "parent", name: "GJC session", nonce } });
			const lease = await journal.claim("create:session:stale", "crashed-owner", 60_000);
			await journal.record("create:session:stale", { owner: "crashed-owner", epoch: lease!.epoch }, "terminal", { provider: "discord", threadId: stale.id, status: "created" });
			const store = new ConversationStore<DiscordConversation>({ agentDir, kind: "discord" });
			const intentKey = "app:guild:parent:creating:session";
			await store.transact(intentKey, () => ({ generation: 1, state: "creating", appId: "app", guildId: "guild", parentChannelId: "parent", sessionId: "session", endpointGeneration: 1, createNonce: "stale", updatedAt: 1, seenEventIds: [], seenInteractionIds: [] }));
			await store.transact("app:guild:parent:replacement", () => ({ generation: 1, state: "active", appId: "app", guildId: "guild", parentChannelId: "parent", threadId: "replacement", sessionId: "session", endpointGeneration: 2, updatedAt: 2, supersededByThreadId: "successor", seenEventIds: [], seenInteractionIds: [] }));
			const restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, resolveEndpoint: async () => ({ generation: 2, isCurrent: () => true, send: () => {} }) });
			await restarted.start();
			expect(await store.read(`app:guild:parent:${stale.id}`)).toBeUndefined();
			expect(await store.read(intentKey)).toBeUndefined();
			expect((await restarted.notify({ sessionId: "session", endpointGeneration: 2, content: "current" })).threadId).toBe("replacement");
			await restarted.stop();
		});
	});
	...
	test("retains action authority when the captured endpoint is replaced or removed before reply", async () => {
		for (const unavailable of ["replaced", "removed"] as const) {
			...
			let deferred = false;
			await withDaemon(async (daemon, provider) => {
				const conversation = await daemon.notify({ sessionId: "session", endpointGeneration: 7, content: "Choose", actionId: "ask", options: ["Yes"] });
				provider.deferInteraction = async () => { if (!deferred) { deferred = true; current = false; } };
				...
				resolveEndpoint: async (): Promise<DiscordEndpointBinding | null> => current
					? { generation: 7, isCurrent: () => current, send: frame => { frames.push(frame); } }
					: unavailable === "removed" ? null : { generation: 8, isCurrent: () => false, send: frame => { frames.push(frame); } },
			});
		}
	});
	...
	test("terminalizes callback-failed and stale action receipts on restart without redelivery", async () => {
		for (const staleBinding of [false, true]) {
			await withDaemon(async (daemon, provider, agentDir) => {
				const conversation = await daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "Choose", actionId: "ask", options: ["Yes"] });
				provider.deferInteraction = async () => { throw new Error("callback failed"); };
				await expect(daemon.handleInbound(inbound(conversation.threadId!, `restart-${staleBinding}`, 1))).rejects.toThrow("callback failed");
				const restarted = new DiscordNotificationDaemon({
					agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider,
					resolveEndpoint: async () => staleBinding ? null : { generation: 1, isCurrent: () => true, send: () => {} },
				});
				await restarted.start();
				...
				const stored = JSON.parse(await fs.readFile(path.join(agentDir, "sdk", "daemons", "discord", "conversations.json"), "utf8")) as { conversations: Record<string, DiscordConversation> };
				const mapping = Object.values(stored.conversations).find(record => record.threadId === conversation.threadId);
	...
	test("retries an interaction after pre-send failure but never resends after cleanup becomes uncertain", async () => {
		...
		await withDaemon(async (daemon, provider) => {
			const conversation = await daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "Choose", actionId: "ask", options: ["Yes"] });
			provider.deferInteraction = async () => { if (deferFails) { deferFails = false; throw new Error("before send"); } };
			...
			const next = await daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "Again", actionId: "ask", options: ["Yes"] });
			...
			expect(frames[1]).toMatchObject({ type: "reply", id: "ask", answer: "yes", idempotencyKey: expect.any(String) });
		}, {
			resolveEndpoint: async (): Promise<DiscordEndpointBinding | null> => ({ generation: 1, isCurrent: () => current, send: frame => { frames.push(frame); current = false; } }),
		});
	});
	...
	test("replays only pre-send receipts across restart and fences a stale claimer", async () => {
		...
		await withDaemon(async (daemon, provider, agentDir) => {
			const endpoint = async (): Promise<DiscordEndpointBinding> => ({ generation: 1, isCurrent: () => true, send: frame => { frames.push(frame); } });
			const first = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, now: () => now, resolveEndpoint: endpoint });
			const conversation = await first.notify({ sessionId: "session", endpointGeneration: 1, content: "Choose", actionId: "ask", options: ["Yes"] });
			provider.deferInteraction = async () => { entered.resolve(); await release.promise; };
			const blocked = first.handleInbound(inbound(conversation.threadId!, "claim-before-send", 1));
			await entered.promise;
			now = 60_001;
			const restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, now: () => now, resolveEndpoint: endpoint });
			await restarted.start();
			...
			const retry = await restarted.notify({ sessionId: "session", endpointGeneration: 1, content: "Again", actionId: "ask", options: ["Yes"] });
			provider.deferInteraction = async () => { throw new Error("definite pre-send"); };
			await expect(restarted.handleInbound(inbound(retry.threadId!, "definite-pre-send", 1))).rejects.toThrow("Discord interaction callback failed");
			const afterPreSend = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, now: () => now, resolveEndpoint: endpoint });
			await afterPreSend.start();
			expect(frames).toHaveLength(0);

			const uncertain = await afterPreSend.notify({ sessionId: "session", endpointGeneration: 1, content: "Last", actionId: "ask", options: ["Yes"] });
			provider.deferInteraction = async () => {};
			const originalSend = (await endpoint()).send;
			const throwingEndpoint = async (): Promise<DiscordEndpointBinding> => ({ generation: 1, isCurrent: () => true, send: frame => { originalSend(frame); throw new Error("accepted then disconnected"); } });
			const sender = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, now: () => now, resolveEndpoint: throwingEndpoint });
			await sender.handleInbound(inbound(uncertain.threadId!, "uncertain-post-send", 1));
			expect(frames).toHaveLength(1);
			const afterUncertain = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, now: () => now, resolveEndpoint: endpoint });
			await afterUncertain.start();
			expect(frames).toHaveLength(1);
		}, { resolveEndpoint: async () => ({ generation: 1, isCurrent: () => true, send: frame => { frames.push(frame); } }) });
	});
	...
	test("rejects stale commands before dispatch", async () => {
		...
		await withDaemon(async (daemon, provider) => {
			...
			await daemon.handleInbound({ ...inbound(conversation.threadId!, "stale-command"), interaction: undefined, content: "/sdk query todo.list {}" });
			expect(commands).toEqual([]);
			expect(provider.messages.at(-1)).toEqual({ threadId: conversation.threadId!, content: "This conversation is no longer available." });
		}, {
			resolveEndpoint: async () => ({ generation: 4, isCurrent: () => true, send: () => {} }),
			onCommand: async (_sessionId, content) => { commands.push(content); return true; },
		});
	});
	...
	test("reconstructs an uncertain post receipt from its stable nonce without a duplicate", async () => {
		await withDaemon(async (daemon, provider, agentDir) => {
			...
			await expect(daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "open" })).rejects.toThrow("Discord disconnected after accepting post");
			expect(provider.messages).toHaveLength(1);
			expect([...provider.messageNonces.keys()][0]).toMatch(/^gjc-[a-f0-9]{21}$/);
			const restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, resolveEndpoint: async () => ({ generation: 1, isCurrent: () => true, send: () => {} }) });
			await restarted.start();
			expect(provider.messages).toHaveLength(1);
			await restarted.stop();
		});
	...
	test("reconciles an accepted action post after disconnect and accepts its published custom ID once", async () => {
		...
		await withDaemon(async (daemon, provider, agentDir) => {
			provider.failPostAfterPersist = true;
			await expect(daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "Choose", actionId: "ask", options: ["Yes"] })).rejects.toThrow("Discord disconnected after accepting post");
			const threadId = provider.messages[0]!.threadId;
			const publishedCustomId = actionCustomIds.get(threadId)!;
			const restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, resolveEndpoint: async () => ({ generation: 1, isCurrent: () => true, send: frame => { frames.push(frame); } }) });
			await restarted.start();
			expect(provider.messages).toHaveLength(1);
			...
			expect(frames).toMatchObject([{ type: "reply", id: "ask", answer: "yes", idempotencyKey: expect.any(String) }]);
			await restarted.handleInbound({ ...inbound(threadId, "duplicate-action", 1), interaction: { id: "interaction-duplicate-action", token: "token-duplicate-action", customId: publishedCustomId, value: "yes" } });
			expect(frames).toHaveLength(1);
			await restarted.stop();
		});
	});
	test("reuses a durable action publication intent after remote acceptance, reconnect, and repeated notification", async () => {
		...
		await withDaemon(async (daemon, provider, agentDir) => {
			...
			const threadId = provider.messages[0]!.threadId;
			const publishedCustomId = actionCustomIds.get(threadId)!;
			const retried = await daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "Choose again", actionId: "ask", options: ["Yes"] });
			...
			const restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, resolveEndpoint: async () => ({ generation: 1, isCurrent: () => true, send: frame => { frames.push(frame); } }) });
			await restarted.start();
			await restarted.handleInbound({ ...inbound(threadId, "original-route", 1), interaction: { id: "interaction-original-route", token: "token-original-route", customId: publishedCustomId, value: "yes" } });
			await restarted.handleInbound({ ...inbound(threadId, "original-route-duplicate", 1), interaction: { id: "interaction-original-route-duplicate", token: "token-original-route-duplicate", customId: publishedCustomId, value: "yes" } });
			expect(frames).toMatchObject([{ type: "reply", id: "ask", answer: "yes", idempotencyKey: expect.any(String) }]);
			expect(frames).toHaveLength(1);
			await restarted.stop();
		});
	...
	test("rebinds an orphaned durable command effect during startup", async () => {
		...
		await withDaemon(async (daemon, provider, agentDir) => {
			...
			await journal.enqueue({
				id: effectId, kind: "discord.inbound.command", transport: "discord", sessionId: "session", endpointGeneration: 1,
				payload: { type: "command", content: "/sdk query recovered", idempotencyKey: effectId, routing: { guildId: "guild", parentId: "parent", threadId: conversation.threadId!, eventId: "orphan-command", kind: "command" } },
			});
			const restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, resolveEndpoint: async () => ({ generation: 1, isCurrent: () => true, send: () => {} }), onCommand: async (_sessionId, command) => { commands.push(command); return true; } });
			await restarted.start();
	...
	test("completes recovery before Gateway delivery", async () => {
		...
		await withDaemon(async (daemon, provider, agentDir) => {
			...
			await new ChatEffectJournal({ agentDir, transport: "discord" }).enqueue({
				id: effectId, kind: "discord.inbound.command", transport: "discord", sessionId: "session", endpointGeneration: 1,
				payload: { type: "command", content: "/sdk recovered", idempotencyKey: effectId, routing: { guildId: "guild", parentId: "parent", threadId: conversation.threadId!, eventId: "recovered", kind: "command" } },
			});
			provider.startEvent = { ...inbound(conversation.threadId!, "early"), interaction: undefined, content: "/sdk early" };
			const restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, resolveEndpoint: async () => ({ generation: 1, isCurrent: () => true, send: () => {} }), onCommand: async (_sessionId, content) => { commands.push(content); return true; } });
			await restarted.start();
	...
	test("terminalizes a rejected orphan action after restart without dispatching it", async () => {
		...
		await withDaemon(async (daemon, provider, agentDir) => {
			...
			await new ChatEffectJournal({ agentDir, transport: "discord" }).enqueue({
				id: effectId, kind: "discord.inbound.action", transport: "discord", sessionId: "session", endpointGeneration: 1,
				payload: { type: "reply", id: "ask", answer: "yes", idempotencyKey: effectId, routing: { guildId: "guild", parentId: "parent", threadId: conversation.threadId!, eventId: "crashed-rejected", interactionId: "interaction-crashed-rejected", kind: "action", actionId: "ask" } },
			});
			await daemon.resolveAction("session", "ask");
			const restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, resolveEndpoint: async () => ({ generation: 1, isCurrent: () => true, send: frame => { frames.push(frame); } }) });
			await restarted.start();
			expect(frames).toEqual([]);
			expect((await new ChatEffectJournal({ agentDir, transport: "discord" }).read(effectId))?.receipt).toEqual({ status: "rejected" });
			await restarted.stop();
		});
	});
	test("terminalizes a mapping-rejected click and never adopts it after action ID reuse", async () => {
		const frames: Record<string, unknown>[] = [];
		await withDaemon(async (daemon, provider, agentDir) => {
			const conversation = await daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "First", actionId: "ask", options: ["Yes"] });
			const oldCustomId = actionCustomIds.get(conversation.threadId!)!;
			await daemon.resolveAction("session", "ask");
			await daemon.handleInbound({ ...inbound(conversation.threadId!, "reused-rejected", 1), interaction: { id: "interaction-reused-rejected", token: "token-reused-rejected", customId: oldCustomId, value: "yes" } });
			const effectId = `discord:app:guild:parent:${conversation.threadId}:reused-rejected`;
			expect((await new ChatEffectJournal({ agentDir, transport: "discord" }).read(effectId))?.receipt).toEqual({ status: "rejected" });
			await daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "Second", actionId: "ask", options: ["Yes"] });
	...
	test("terminalizes stale post, archive, and unarchive effects after close and generation-rotated resume", async () => {
		await withDaemon(async (daemon, provider, agentDir) => {
			const original = await daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "open" });
			const journal = new ChatEffectJournal({ agentDir, transport: "discord" });
			await journal.enqueue({ id: "stale-post", kind: "post-message", transport: "discord", sessionId: "session", endpointGeneration: 1, payload: { threadId: original.threadId!, content: "must not post", nonce: "stale-post" } });
			await daemon.close("session");
			await journal.enqueue({ id: "stale-archive", kind: "archive", transport: "discord", sessionId: "session", endpointGeneration: 1, payload: { threadId: original.threadId! } });
			const archived = await daemon.notify({ sessionId: "resumed-session", endpointGeneration: 1, content: "open resumed" });
			await daemon.archive("resumed-session");
			await journal.enqueue({ id: "stale-unarchive", kind: "unarchive", transport: "discord", sessionId: "resumed-session", endpointGeneration: 1, payload: { threadId: archived.threadId! } });
			...
			await Bun.sleep(50);
			for (const id of ["stale-post", "stale-archive", "stale-unarchive"]) expect((await journal.read(id))?.state).toBe("terminal");
	...
	test("propagates callback defer failures through Gateway delivery after recording the receipt", async () => {
		await withDaemon(async (daemon, provider, agentDir) => {
			const conversation = await daemon.notify({ sessionId: "session", endpointGeneration: 1, content: "Choose", actionId: "ask", options: ["Yes"] });
			provider.deferInteraction = async () => { throw new Error("callback unavailable"); };
			...
			expect((await new ChatEffectJournal({ agentDir, transport: "discord" }).read(effectId))?.receipt).toEqual({ status: "callback_failed" });
		});
	});
	test("recovers a dead unexpired inbound lease without another gateway event and cancels it on stop", async () => {
		...
		try {
			...
			await journal.enqueue({
				id: leasedEffectId, kind: "discord.inbound.command", transport: "discord", sessionId: "session", endpointGeneration: 1,
				payload: { type: "command", content: "/sdk query recovered", idempotencyKey: leasedEffectId, routing: { guildId: "guild", parentId: "parent", threadId: conversation.threadId!, eventId: "leased-command", kind: "command" } },
			});
			await journal.claim(leasedEffectId, "dead-worker", 10);
			const commands: string[] = [];
			restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, now: () => now, resolveEndpoint: endpoint, onCommand: async (_sessionId, command) => { commands.push(command); return true; } });
			await restarted.start();
			...
			await journal.enqueue({
				id: stoppedEffectId, kind: "discord.inbound.command", transport: "discord", sessionId: "session", endpointGeneration: 1,
				payload: { type: "command", content: "/sdk query stopped", idempotencyKey: stoppedEffectId, routing: { guildId: "guild", parentId: "parent", threadId: conversation.threadId!, eventId: "stopped-command", kind: "command" } },
			});
			now = 20;
			await journal.claim(stoppedEffectId, "dead-worker", 10);
			const stoppedCommands: string[] = [];
			restarted = new DiscordNotificationDaemon({ agentDir, repo: agentDir, guildId: "guild", parentChannelId: "parent", provider, now: () => now, resolveEndpoint: endpoint, onCommand: async (_sessionId, command) => { stoppedCommands.push(command); return true; } });
			await restarted.start();
			await restarted.stop();
			restarted = undefined;
			now = 31;
