...
function successfulStream(model: Model): AssistantMessageEventStream {
	return createMockModel({ responses: [{ content: ["Profile fallback accepted"] }] }).stream(model, { systemPrompt: [], messages: [], tools: [] });
}
...
describe("model profile fallback chain e2e", () => {
	...
	it("activates a preset role array through fallback, sticks it for the session, and retains the selector chain", async () => {
		...
		expect(switches).toEqual([expect.objectContaining({ from: selector(primary), to: selector(fallback), role: "default", activeIndex: 1, chainLength: 2 })]);
