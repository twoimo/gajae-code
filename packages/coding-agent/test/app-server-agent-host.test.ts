...
describe("AgentSessionHost", () => {
	...
	it("catalogs builtin, file, skill, and extension slash commands with classifications and extension state depth", async () => {
		...
		try {
			...
			expect(Object.hasOwn(skillExtension, "disabledReason")).toBe(true);
			expect(Object.hasOwn(skillExtension, "shadowedBy")).toBe(true);
		} finally {
	...
	it("emits jobs_changed when job observer reports status transitions", async () => {
		...
		expect(emitted).toEqual([
			...
			[
				...
				1,
				"jobs_changed",
				{ kind: "agent", id: "a2", status: "running", description: "agent slice" },
			],
			...
			[
				...
				1,
				"jobs_changed",
				{ kind: "agent", id: "a2", status: "cancelled", description: "agent slice" },
			],
		]);
	});
	...
	it("does not emit duplicate job changes for unchanged snapshots and keeps same job ids isolated by thread", async () => {
		...
		a.jobSnapshot = {
			running: [{ id: "shared", type: "bash", status: "running", label: "A", metadata: {} }],
			recent: [],
		};
		b.jobSnapshot = {
			running: [{ id: "shared", type: "bash", status: "running", label: "B", metadata: {} }],
			recent: [],
		};
		a.fireJobChange();
		b.fireJobChange();
		...
		a.jobSnapshot = {
			running: [],
			recent: [{ id: "shared", type: "bash", status: "completed", label: "A", metadata: {} }],
		};
		a.fireJobChange();
	...
	it("emits each rapid job terminal transition once", async () => {
		...
		await host.createThread({});
		session.jobSnapshot = {
			running: [{ id: "j-fast", type: "bash", status: "running", label: "fast", metadata: {} }],
			recent: [],
		};
		session.fireJobChange();
		session.jobSnapshot = {
			running: [],
			recent: [{ id: "j-fast", type: "bash", status: "completed", label: "fast", metadata: {} }],
		};
		session.fireJobChange();
		session.jobSnapshot = {
			running: [],
			recent: [{ id: "j-fast", type: "bash", status: "failed", label: "fast", metadata: {} }],
		};
		session.fireJobChange();
	...
	it("unsubscribes job observers on dispose", async () => {
		...
		session.jobSnapshot = {
			running: [{ id: "late", type: "bash", status: "running", label: "late", metadata: {} }],
			recent: [],
		};
		session.fireJobChange();
		expect(emitted).toEqual([]);
	...
	it("maps monitor cron snapshots and bounded output tails from retained async job output", async () => {
		...
		const manager = new AsyncJobManager({ onJobComplete: () => undefined });
		AsyncJobManager.setInstance(manager);
		const jobId = manager.register("bash", "monitor with output", () => new Promise(() => undefined), {
			id: "m1",
			ownerId: "0-Test",
			metadata: { monitor: true, kind: "log" } as never,
		});
		manager.appendOutput(
			jobId,
			`${"x".repeat(4100)}sk-12345678901234567890 Bearer secret-token api_key=secret-value`,
		);
		session.jobSnapshot = {
			running: [
				{
	...
	it("caps pending events and clears the buffer on dispose before generation sync", async () => {
		...
		try {
			...
			for (let i = 0; i < 513; i += 1) {
				sessions[1]?.events[0]?.({ type: `pending_${i}` } as unknown as AgentSessionEvent);
			}
			...
			sessions[2]?.events[0]?.({ type: "pending_before_dispose" } as unknown as AgentSessionEvent);
	...
	it("buffers job changes during generation-zero reattach and clears them on dispose", async () => {
		...
		await host.createThread({});
		const reopened = await host.createThread({});
		sessions[1]!.jobSnapshot = {
			running: [
				{ id: "during-reattach", type: "bash", status: "running", label: "tail", metadata: { monitor: true } },
			],
			recent: [],
		};
		...
		expect(emitted).toEqual([
			[
				...
				7,
				"jobs_changed",
				{ kind: "monitor", id: "during-reattach", status: "running", description: "tail" },
			],
		]);

		const disposed = await host.createThread({});
		sessions[2]!.jobSnapshot = {
			running: [{ id: "discarded", type: "bash", status: "running", label: "drop", metadata: {} }],
			recent: [],
		};
	...
	it("routes Rust backend payload shapes to session methods", async () => {
		...
		expect(session.models).toEqual([
			{ model: { provider: "anthropic", modelId: "claude-opus" }, role: undefined, options: undefined },
		]);
	...
	it("assigns model roles with TUI-compatible target semantics", async () => {
		...
		expect(
			await host.backendCall(created.threadId, "modelAssign", {
				role: "executor",
				provider: "openai",
				modelId: "gpt-4.1",
			}),
		).toEqual({ ok: true, role: "executor", modelId: "gpt-4.1" });
		expect(session.models).toEqual([]);
		...
		).toEqual({ ok: true, role: "architect", modelId: "gpt-4.1" });
		expect(session.models).toEqual([]);
		expect(session.settingsValues["task.agentModelOverrides"]).toEqual({
			executor: "openai/gpt-4.1",
			architect: "openai/gpt-4.1:high",
		});
		...
		).toEqual({ ok: true, role: "default", modelId: "claude-opus" });
		expect(session.models).toEqual([
			{
				model: { provider: "anthropic", id: "claude-opus", name: "Claude Opus" },
				role: "default",
				options: { selector: "anthropic/claude-opus", thinkingLevel: "high" },
			},
		]);
		...
		await expect(
			host.backendCall(created.threadId, "modelAssign", { role: "smol", provider: "openai", modelId: "gpt-4.1" }),
		).rejects.toThrow("unsupported model assignment role: smol");
		await expect(
			host.backendCall(created.threadId, "modelAssign", { role: "default", provider: "missing", modelId: "none" }),
		).rejects.toThrow("unknown model: missing/none");
	...
	it("keeps compatibility fallbacks for legacy backend payload shapes", async () => {
		...
		expect(session.models).toEqual([
			{ model: { provider: "openai", modelId: "gpt-4.1" }, role: undefined, options: undefined },
		]);
	...
	it("persists settings updates through the session settings owner", async () => {
		...
		try {
			...
			} as unknown as FakeSession["settings"];
			const host = new AgentSessionHost({ sessionFactory: async () => ({ session }) });
			await host.createThread({});
	...
	it("re-opening the same session replaces the old subscription and uses backend-assigned generation", async () => {
		...
		sessions[0]?.events[0]?.({ type: "stale" } as unknown as AgentSessionEvent);
		sessions[1]?.events[0]?.({ type: "fresh" } as unknown as AgentSessionEvent);
	...
	it("lists and fuzzy-searches sessions scoped by cwd, path, and deep message text", async () => {
		...
		try {
			...
			expect(deep.sessions[0]?.path).toBe(managerA.getSessionFile() as string);
		...
		try {
			const storage = {
				login: async (_providerId: string, callbacks: any) => {
					callbacks.onAuth({
						authUrl: "https://login.example/authorize?fingerprint=fingerprint_g004_secret",
						instructions: "Open browser fingerprint=fingerprint_g004_secret token=tok_g004_secret",
					});
			...
			const complete = (await host.authLoginComplete({
				flowId: start.flowId,
				redirectUrl: "http://localhost/callback?code=code_g004_secret&state=verifier_g004_secret",
			})) as any;
			...
			const cancelStorage = {
				login: async (_providerId: string, callbacks: any) => {
					...
					await new Promise((_resolve, reject) =>
						callbacks.signal.addEventListener("abort", () => reject(new Error("cancelled"))),
					);
				},
			};
