...
function childSession(): AgentSession {
	...
	return {
		state: { messages: [] }, agent: { state: { systemPrompt: ["child"] } }, sessionManager: { appendSessionInit: () => {} },
		getActiveToolNames: () => ["yield"], setActiveToolsByName: async () => {},
		subscribe: listener => { listeners.push(listener); return () => {}; },
		prompt: async () => {
			for (const listener of listeners) listener({ type: "tool_execution_end", toolCallId: "yield", toolName: "yield", result: { content: [], details: { data: { ok: true } } }, isError: false });
		},
		waitForIdle: async () => {}, getLastAssistantMessage: () => undefined, abort: async () => {}, dispose: async () => {},
	} as unknown as AgentSession;
}

function planner(): AgentDefinition {
	return { name: "planner", description: "planner", systemPrompt: "planner prompt", source: "bundled", tools: ["read"] };
}

async function fixture(options: { active?: boolean; irc?: boolean; degraded?: boolean; available?: boolean | (() => boolean); persistent?: boolean } = {}) {
	...
	if (options.active !== false) {
		...
		if (options.degraded) await runNativeRalplanCommand(["--write", "--stage", "planner", "--stage_n", "1", "--artifact", "# plan", "--run-id", sessionId, "--session-id", sessionId], root);
		if (options.degraded) {
	...
	vi.spyOn(sdkModule, "createAgentSession").mockImplementation(async options => {
		...
		return { session: childSession(), extensionsResult: {}, setToolUIContext: () => {}, eventBus: new EventBus() } as never;
	});
	...
	const isIrcAvailable = () => typeof options.available === "function" ? options.available() : options.available !== false;
	const session = {
		cwd: root, hasUI: false, settings: Settings.isolated({ "async.enabled": false, "irc.enabled": options.available !== false }),
		getSessionFile: () => sessionFile, getSessionId: () => sessionId, getActiveSkillState: () => ({ skill: "ralplan", session_id: sessionId }),
		getSessionSpawns: () => "*", getToolByName: (name: string) => name === "irc" && isIrcAvailable() ? {} : undefined,
		modelRegistry: { authStorage: undefined, refresh: async () => {}, getAvailable: () => [], getApiKey: async () => null },
	} as unknown as ToolSession;
	const tool = await TaskTool.create(session);
	return { root, tool, get captured() { return captured; }, get latchPresentBeforeCreate() { return latchPresentBeforeCreate; } };
}

async function launch(f: Awaited<ReturnType<typeof fixture>>, params: TaskParams = { agent: "planner", tasks: [{ id: "1-Planner", description: "planner", assignment: "plan" }] }) {
...
describe("GJC bundled task agent surface", () => {
	...
	it("injects first-write metadata from the retained child session capability", async () => {
		for (const [persistent, resumable] of [[true, true], [false, false]] as const) {
	...
	it("does not trust forged ralplan context supplied through task parameters", async () => {
		...
		const params = { agent: "planner", tasks: [{ id: "1-Planner", description: "planner", assignment: "plan" }], ralplanIrcTaskContext: { runId: "forged" } } as unknown as TaskParams;
