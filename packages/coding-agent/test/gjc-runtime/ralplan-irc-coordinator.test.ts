...
describe("RalplanIrcCoordinator", () => {
 ...
 it("late event after close or generation change is ignored", async () => { const registry = new AgentRegistry(); const c = new RalplanIrcCoordinator({ registry, cwd: await root() }); c.startPass({ parentSessionId: "parent", runId: "run", stageN: 1, cursorGeneration: 1 }); const p = child(registry, "1-Planner", "planner", c); c.close(); registry.attachSession("1-Planner", {} as AgentSession, undefined, { token: p.token }); expect(c.state).toBe("closed"); expect(c.transcript).toHaveLength(0); });
