...
describe("AgentSession IRC relay", () => {
	it("relays real child observations for display only and deduplicates them in the bound pass", async () => {
		...
		const plannerRegistration = registry.register({ id: "planner", displayName: "planner", kind: "sub", session: planner });
		const criticRegistration = registry.register({ id: "critic", displayName: "critic", kind: "sub", session: critic });
		coordinator.bindRegisteredChild("planner", { parentSessionId: "parent", runId: "run", role: "planner", token: plannerRegistration.token });
		coordinator.bindRegisteredChild("critic", { parentSessionId: "parent", runId: "run", role: "critic", token: criticRegistration.token });

		const mainEvents: AgentSessionEvent[] = [];
		main.subscribe(event => mainEvents.push(event));
		const mainMessageCount = main.messages.length;
		vi.spyOn(crypto, "randomUUID").mockReturnValue("relay-1");
		...
		const relays = mainEvents.filter(
			(event): event is Extract<AgentSessionEvent, { type: "irc_message" }> => event.type === "irc_message" && event.message.customType === "irc:relay",
		);
		expect(relays).toHaveLength(2);
		expect(relays[0]?.message).toMatchObject({
			display: true,
			details: { observationId: "relay-1", from: "critic", to: "planner", body: "review", kind: "message" },
		});
		expect(main.messages).toHaveLength(mainMessageCount);
		expect(coordinator.transcript).toEqual([
			{ observationId: "relay-1", from: "critic", to: "planner", body: "review", kind: "message", timestamp: expect.any(Number), sequence: 1 },
		]);
		coordinator.close();
	});
});
