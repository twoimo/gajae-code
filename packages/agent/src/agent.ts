...
export class Agent {
	...
	async #runLoop(messages?: AgentMessage[], options?: AgentPromptOptions & { skipInitialSteeringPoll?: boolean }) {
		...
		if (fallbackManaged && this.#cursorToolResultBuffer.length > 0) {
			const error = new ManagedCursorInvariantError("Managed Cursor attempt started with buffered provider-side tool results");
			this.#state.isStreaming = false;
			this.#abortController = undefined;
			this.#activeRunId = undefined;
			this.#runningPrompt = undefined;
			...
			...(fallbackManaged
				? {
						...
						onManagedAttemptOutcome: async outcome => {
							...
							managedDecision =
								(await options?.onManagedAttemptOutcome?.(outcome)) ??
								{ type: "terminal", terminal: { stopReason: outcome.type === "run_terminal" ? outcome.reason : "error" } };
		...
		try {
			...
			if (!continuation || !ownership.isCurrent()) return;
			try {
	...
	#finalizeRun(logicalRunId: ManagedLogicalRunId, event?: Extract<AgentEvent, { type: "agent_end" }>, beforeEvent?: () => void): void {
