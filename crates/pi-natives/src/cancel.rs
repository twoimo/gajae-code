...
impl Flag {
	...
	fn abort(&self, reason: AbortReason) {
		if self.reason.swap(reason as u8, Ordering::SeqCst) == 0 {
...
impl CancelToken {
	...
	pub fn heartbeat(&self) -> Result<()> {
		if let Some(reason) = self.flag.as_ref().and_then(|flag| flag.cause()) {
	...
	pub async fn wait(&self) -> AbortReason {
		...
		tokio::select! { reason = by_flag => reason, reason = by_timeout => reason }
	}
	...
	pub fn aborted(&self) -> bool {
		self.flag.as_ref().and_then(|flag| flag.cause()).is_some()
			|| self
				.deadline
				.is_some_and(|deadline| deadline < Instant::now())
	}
...
impl AbortToken {
	pub fn abort(&self, reason: AbortReason) {
		if let Some(flag) = self.0.as_ref().and_then(Weak::upgrade) {
			flag.abort(reason);
		}
	}
}
