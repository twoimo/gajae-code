...
compaction, and remote notifications. Teams keep rebuilding these from scratch.

Gajae-Code packages that entire runtime as a reusable component:
...
- **Remote comes for free.** Every session hosts a loopback WebSocket endpoint.
  The moment your app embeds Gajae-Code, Telegram/Discord/Slack notifications,
  phone-answerable asks, CLI automation, and Coordinator MCP orchestration all
  work without you writing any of it.
- **Many subscribers, one session.** The event stream supports multiple
  subscribers: your app UI, a remote notifier, and an audit logger can all watch
...
subscriber (full-fidelity streaming), while remote channels attach over
WebSocket for notifications and approvals.
...
execution runtime. All of the following are `createAgentSession()` options
(see `CreateAgentSessionOptions` in `packages/coding-agent/src/sdk/session.ts`):
...
`usage.get`, `models.list/current`, `workflow.gates.list`, …). The full catalog
lives in `packages/coding-agent/src/sdk/protocol/operation-registry.ts`.

## Creating sessions programmatically

You do not need a pre-existing session. The SDK broker starts on demand:

```bash
gjc daemon session --op session.create ...
```

or from code: `ensureBroker()` → connect → `session.create` /`fork`/`resume`/
`close`/`delete` (global operations, idempotency-key protected). Multi-session
controllers can also use the Coordinator MCP (`gjc mcp-serve coordinator`).
...
- **Vertical AI app (delete your agentic loop).** Embed with `toolNames` +
  ...
  in-process for token-level streaming. Users automatically get phone
  notifications and remote approvals via the bundled adapters.
...
- **Mobile / desktop companion.** Same WS contract the Telegram daemon uses:
  discover endpoints, render `action_needed`, send `reply`. Threaded frames give
  you live activity and context updates.
- **Fleet orchestrator.** Coordinator MCP or broker global operations to create
  and supervise many worktree-scoped sessions.
