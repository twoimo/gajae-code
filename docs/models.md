...
- `codex-pro` — GPT-5.6 Sol `xhigh` for the main session, `high` for planner, and `max` for critic and architect, with Terra `medium` for executor
...
The `eco` tier favors cost and latency, `medium` balances capability and cost, and `pro` reserves the strongest native reasoning for planning and review. GPT-5.6 provider requests support canonical efforts from `low` through `max`; unsupported effort names fail validation instead of being silently remapped. Effort suffixes are clamped to each model's supported thinking range at preview and activation time. Single-provider tiers pin each provider's current flagship (`zai/glm-5.2`, `kimi-code/kimi-k2.7-code`, `xiaomi/mimo-v2.5-pro`, `xai/grok-4.3`, `cursor/composer-1.5`, `minimax-code/minimax-m3`). User-defined profiles override built-ins by exact profile name.
...
```sh
gjc --mpreset codex-pro --default
gjc --mpreset opencodego
```
