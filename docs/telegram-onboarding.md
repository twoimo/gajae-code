...
- send in-topic config commands:
  ...
  Finalized `/btw` replies promote supported Markdown tables to native Telegram rich-table blocks by default. Inline `$...$` or `\(...\)` formulas, including table cells, use Telegram mathematical-expression rich text; standalone `$$...$$` or `\[...\]` formulas use mathematical-expression blocks. Unbalanced or ambiguous formulas, unsupported Markdown, table limits (20 columns, 500 blocks, and 16 nesting levels), rich-delivery failures, and replies over 32,768 characters retain the original Markdown or existing HTML fallback; `/rich off` keeps the HTML-only path.
